...
export default function Billing({
  ...
  const [showPrintBillsList, setShowPrintBillsList] = useState(false);
  ...
  React.useEffect(() => {
    if (initialJobForInvoice) {
      ...
      if (existingInv) {
        ...
        const parts = Array.isArray(initialJobForInvoice.partsUsed) ? initialJobForInvoice.partsUsed : [];
        const partsCost = parts.reduce((sum: number, p: any) => sum + (Number(p.qty || 1) * Number(p.unitPrice || p.rate || 0)), 0);
        const finalAmt = initialJobForInvoice.finalBillAmount !== undefined ? initialJobForInvoice.finalBillAmount : (initialJobForInvoice.estimateAmount || 0);
        const serviceCharge = Math.max(0, finalAmt - partsCost);
        ...
        if (serviceCharge > 0 || parts.length === 0) {
          items.push({
            productName: desc,
            serialNo: initialJobForInvoice.serialNo || initialJobForInvoice.id,
            qty: 1,
            rate: parts.length > 0 ? serviceCharge : finalAmt
          });
        }
        parts.forEach((p: any) => {
          items.push({
            productName: p.productName || 'Spare Part',
            serialNo: p.serialNo || initialJobForInvoice.serialNo || 'N/A',
            qty: Number(p.qty || 1),
  ...
  // Invoices strictly scoped by tenant
  const tenantInvoices = [...invoices].sort((first, second) => {
    ...
    if (isAdmin || activeTenantId === 'org-admin') {
      return inv.tenantId === 'org-admin';
    }
    return inv.tenantId === activeTenantId || !inv.tenantId;
  });

  const totalGstCount = tenantInvoices.filter(inv => (inv.taxPercent || 0) > 0 || (inv.taxAmount || 0) > 0).length;
  const totalNonGstCount = tenantInvoices.filter(inv => (!inv.taxPercent || inv.taxPercent === 0) && (!inv.taxAmount || inv.taxAmount === 0)).length;

  // Filter invoices strictly by tenant context, search term, and GST filter
  const filteredInvoices = tenantInvoices.filter(inv => {
    ...
    const isGstBill = (inv.taxPercent || 0) > 0 || (inv.taxAmount || 0) > 0;
    if (gstFilter === 'gst') {
      return isGstBill;
  ...
  const filteredSubtotalSum = filteredInvoices.reduce((acc, inv) => acc + (inv.subtotal || 0), 0);
  const filteredTaxSum = filteredInvoices.reduce((acc, inv) => acc + (inv.taxAmount || 0), 0);
  const filteredGrandTotalSum = filteredInvoices.reduce((acc, inv) => acc + (inv.grandTotal || 0), 0);
  const filteredPaidSum = filteredInvoices.reduce((acc, inv) => acc + (inv.paidAmount || 0), 0);
  const filteredBalanceSum = filteredInvoices.reduce((acc, inv) => acc + (inv.balanceAmount || 0), 0);

  const handleExportCsv = () => {
    const headers = ['Invoice No', 'Date', 'Client Name', 'Mobile', 'Linked Job', 'Taxable Amount', 'GST Rate %', 'GST Amount', 'Grand Total', 'Paid Amount', 'Balance Due', 'Status', 'Payment Mode'];
    const rows = filteredInvoices.map(inv => [
      ...
      `"${inv.isPaid ? 'Paid' : (inv.balanceAmount > 0 ? 'Unpaid' : 'Paid')}"`,
      `"${inv.paymentMode || ''}"`
    ]);
    ...
    const filterLabel = gstFilter === 'gst' ? 'GST_Bills' : gstFilter === 'non_gst' ? 'Non_GST_Bills' : 'All_Bills';
      ...
      {!showCreateInvoice ? (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
          <div className="p-4 border-b border-slate-100 bg-slate-50/40 flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            {/* GST Filter Tabs */}
            <div className="flex items-center gap-1.5 bg-slate-200/80 p-1 rounded-xl text-xs font-bold w-fit">
              <button
                type="button"
              ...
              >
                <span>All Bills</span>
                <span className="bg-slate-100 text-slate-600 text-[10px] px-1.5 py-0.2 rounded-full border border-slate-200">
              ...
              >
                ...
                <span className={`text-[10px] px-1.5 py-0.2 rounded-full border ${
                  gstFilter === 'gst' ? 'bg-emerald-700/50 text-white border-emerald-500' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
                }`}>
                  {totalGstCount}
                </span>
              </button>
              ...
              >
                ...
                <span className={`text-[10px] px-1.5 py-0.2 rounded-full border ${
                  gstFilter === 'non_gst' ? 'bg-amber-700/50 text-white border-amber-500' : 'bg-amber-50 text-amber-700 border-amber-200'
                }`}>
                  {totalNonGstCount}
                </span>
              </button>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title="Download filtered bills list as CSV"
                className="flex items-center gap-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 text-xs font-semibold px-3 py-2 rounded-xl transition cursor-pointer shadow-2xs"
              >
              ...
              <button
                type="button"
                onClick={() => setShowPrintBillsList(true)}
                title="Print or preview filtered bills list"
                className="flex items-center gap-1.5 bg-teal-50 hover:bg-teal-100 text-teal-700 border border-teal-200 text-xs font-semibold px-3 py-2 rounded-xl transition cursor-pointer shadow-2xs"
              >
          ...
          <div className="border border-slate-100 rounded-xl p-3 bg-slate-50/50 space-y-2.5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {/* Quick Add Product directly into Inventory & Bill */}
                <button
                  type="button"
                  onClick={() => setShowQuickAddProduct(true)}
                  className="inline-flex items-center gap-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 hover:text-emerald-800 border border-emerald-300 font-bold px-2 py-0.5 rounded-lg text-xs transition cursor-pointer shadow-2xs"
                  title="Directly buy and add a part to inventory & bill"
                >
  ...
  {billingSection === 'pricing' && (
    ...
    />
      ...
      {/* Reusable Unified Add Product Modal */}
      <AddProductModal
        ...
        onSuccess={(createdProd) => {
          if (createdProd) {
            const skuVal = createdProd.hsnCode ? `HSN:${createdProd.hsnCode}` : (createdProd.model || createdProd.id || 'N/A');
            // Auto add to active invoice items
            setInvoiceItems(prev => [
              ...prev,
              {
                ...
                serialNo: skuVal,
                qty: 1,
                rate: createdProd.price || 0
              }
            ]);
            // Pre-fill quick draft fields
            setCustomItemName(createdProd.name);
            setCustomItemSku(skuVal);
            setCustomItemRate(createdProd.price || 0);
            setCustomItemQty(1);
          }
      ...
      {/* Bills List Print & Preview Register Modal */}
      {showPrintBillsList && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-2 sm:p-4 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-sm sm:text-base tracking-tight">
                    {gstFilter === 'gst' ? 'GST Tax Bills Register' : gstFilter === 'non_gst' ? 'Non-GST Retail Bills Register' : 'Invoices & Billing Register'}
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    {companyConfig.name} • {filteredInvoices.length} Bills • Generated on {new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={handleExportCsv}
                  className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold px-3 py-1.5 rounded-lg transition cursor-pointer border border-slate-700"
                >
                ...
                <button
                  type="button"
                  onClick={() => window.print()}
                  className="flex items-center gap-1.5 bg-teal-600 hover:bg-teal-500 text-white text-xs font-semibold px-3.5 py-1.5 rounded-lg transition cursor-pointer shadow-xs"
                >
                  <Printer className="w-3.5 h-3.5" />
                ...
                <button
                  type="button"
                  onClick={() => setShowPrintBillsList(false)}
            ...
            <div className="p-4 sm:p-6 bg-slate-100 overflow-y-auto flex-1 text-slate-800">
              <div id="bills-list-print-area" className="printable-area bg-white p-6 rounded-xl border border-slate-200 shadow-xs space-y-4">
                {/* Header Info */}
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-slate-200 pb-4">
                  <div>
                    <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">{companyConfig.name}</h2>
                    <p className="text-xs text-slate-500 mt-0.5">{companyConfig.address}</p>
                    <p className="text-xs text-slate-500">Phone: {companyConfig.phone} | Email: {companyConfig.email}</p>
                    {companyConfig.gstin && (
                      <p className="text-xs font-bold text-slate-700 mt-0.5">
                        GSTIN: <span className="font-mono text-teal-700">{companyConfig.gstin}</span>
                      </p>
                    )}
                  </div>
                  <div className="sm:text-right bg-slate-50 sm:bg-transparent p-3 sm:p-0 rounded-lg border sm:border-0 border-slate-200">
                    <span className="inline-block bg-teal-600 text-white text-[10px] font-black uppercase px-2.5 py-0.5 rounded tracking-wider">
                      {gstFilter === 'gst' ? 'GST Report' : gstFilter === 'non_gst' ? 'Non-GST Report' : 'Complete Register'}
                    </span>
                    <p className="text-xs font-bold text-slate-800 mt-1">Total Records: {filteredInvoices.length}</p>
                    <p className="text-[11px] text-slate-400 font-mono">Date: {new Date().toLocaleDateString('en-IN')}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Taxable Subtotal</span>
                    <span className="text-sm font-black text-slate-800 font-mono">₹{filteredSubtotalSum.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="bg-emerald-50/60 p-3 rounded-lg border border-emerald-200">
                    <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-wider block">Total GST Tax</span>
                    <span className="text-sm font-black text-emerald-700 font-mono">₹{filteredTaxSum.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="bg-teal-50/60 p-3 rounded-lg border border-teal-200">
                    <span className="text-[10px] font-bold text-teal-700 uppercase tracking-wider block">Grand Turnover</span>
                    <span className="text-sm font-black text-teal-900 font-mono">₹{filteredGrandTotalSum.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="bg-amber-50/60 p-3 rounded-lg border border-amber-200">
                    <span className="text-[10px] font-bold text-amber-700 uppercase tracking-wider block">Balance Pending</span>
                    <span className="text-sm font-black text-amber-800 font-mono">₹{filteredBalanceSum.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left border-collapse text-[11px]">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {filteredInvoices.length > 0 ? (
                        filteredInvoices.map((inv, idx) => {
                          const isGst = (inv.taxPercent || 0) > 0 || (inv.taxAmount || 0) > 0;
                          return (
                            <tr key={inv.id} className="hover:bg-slate-50/60">
                              <td className="py-2 px-3 text-slate-400 font-mono">{idx + 1}</td>
                              <td className="py-2 px-3 font-mono font-bold text-slate-900">{inv.id}</td>
                              <td className="py-2 px-3 text-slate-600 whitespace-nowrap">{inv.date || '—'}</td>
                              <td className="py-2 px-3">
                                <p className="font-bold text-slate-800">{inv.clientName}</p>
                                {inv.clientMobile && <p className="text-[10px] text-slate-400 font-mono">{inv.clientMobile}</p>}
                              </td>
                              <td className="py-2 px-3 text-slate-500 font-mono">{inv.linkedJobId || '—'}</td>
                              <td className="py-2 px-3 text-right font-mono font-semibold">₹{(inv.subtotal || 0).toFixed(2)}</td>
                              <td className="py-2 px-3 text-right font-mono">
                                {isGst ? (
                                  <span className="text-emerald-700 font-semibold">
                                    ₹{(inv.taxAmount || 0).toFixed(2)} <span className="text-[9px] text-slate-400">({inv.taxPercent}%)</span>
                                  </span>
                                ) : (
                                  <span className="text-slate-400">0.00</span>
                                )}
                              </td>
                              <td className="py-2 px-3 text-right font-mono font-bold text-slate-900">₹{(inv.grandTotal || 0).toFixed(2)}</td>
                              <td className="py-2 px-3 text-right font-mono text-emerald-700">₹{(inv.paidAmount || 0).toFixed(2)}</td>
                              <td className="py-2 px-3 text-right font-mono font-bold text-rose-700">₹{(inv.balanceAmount || 0).toFixed(2)}</td>
                              <td className="py-2 px-3 text-center whitespace-nowrap">
                                <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                                  inv.isPaid || inv.balanceAmount <= 0
                                    ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                                    : 'bg-rose-50 text-rose-700 border border-rose-200'
                                }`}>
                                  {inv.isPaid || inv.balanceAmount <= 0 ? 'PAID' : 'DUE'}
                                </span>
                              </td>
                            </tr>
                          );
                    ...
                    <tfoot>
                      <tr className="bg-slate-100 font-bold text-slate-900 text-[10px] uppercase border-t-2 border-slate-300">
                        <td colSpan={5} className="py-2.5 px-3">Total ({filteredInvoices.length} Bills)</td>
                        <td className="py-2.5 px-3 text-right font-mono">₹{filteredSubtotalSum.toFixed(2)}</td>
                        <td className="py-2.5 px-3 text-right font-mono text-emerald-700">₹{filteredTaxSum.toFixed(2)}</td>
                        <td className="py-2.5 px-3 text-right font-mono text-teal-800">₹{filteredGrandTotalSum.toFixed(2)}</td>
                        <td className="py-2.5 px-3 text-right font-mono text-emerald-700">₹{filteredPaidSum.toFixed(2)}</td>
                        <td className="py-2.5 px-3 text-right font-mono text-rose-700">₹{filteredBalanceSum.toFixed(2)}</td>
                        <td></td>
                      </tr>
                    </tfoot>
                  </table>
                ...
                <div className="pt-2 text-center text-[10px] text-slate-400">
                  This is a computer-generated register report for {companyConfig.name}.
                </div>
              </div>
            </div>
          </div>
