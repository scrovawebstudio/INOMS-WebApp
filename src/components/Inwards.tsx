...
import { RepairJob, Client, Equipment, Problem, Product, JobStatus, CompanyConfig, ClientType, SystemUser, ServicePartner, sortJobsByLatest } from '../types';
...
interface InwardsProps {
  ...
  onAddProblem?: (name: string) => void | Promise<void>;
  onAddServicePartner?: (partner: Omit<ServicePartner, 'id'>) => void;
...
export default function Inwards({
  ...
  // External Repair / Service Partner Assignment States
  const [repairType, setRepairType] = useState<'Internal' | 'External Service Partner'>('Internal');
  const [servicePartnerId, setServicePartnerId] = useState<string>('');
  const [servicePartnerName, setServicePartnerName] = useState<string>('');
  const [servicePartnerCost, setServicePartnerCost] = useState<number>(0);
  const [partnerSentDate, setPartnerSentDate] = useState<string>('');
  const [partnerExpectedReturnDate, setPartnerExpectedReturnDate] = useState<string>('');
  const [partnerChallanNumber, setPartnerChallanNumber] = useState<string>('');
  const [partnerStatus, setPartnerStatus] = useState<string>('Sent to Service Partner');
  const [partnerRemarks, setPartnerRemarks] = useState<string>('');
  const [partnerWorkDescription, setPartnerWorkDescription] = useState<string>('');

  // Quick Inline Problem Add
  const [showInlineAddProblem, setShowInlineAddProblem] = useState(false);
  const [inlineProblemName, setInlineProblemName] = useState('');

  // Quick Service Partner Add
  const [showQuickAddPartner, setShowQuickAddPartner] = useState(false);
  const [quickPartnerName, setQuickPartnerName] = useState('');
  const [quickPartnerMobile, setQuickPartnerMobile] = useState('');
  const [quickPartnerCity, setQuickPartnerCity] = useState('');

  // Report Hub Filter States
  ...
  const filteredJobs = sortJobsByLatest(inwardJobs.filter(j => {
    ...
    const matchesSearch =
      ...
      (j.productModel || '').toLowerCase().includes(term) ||
      (j.serialNo || '').toLowerCase().includes(term) ||
      (j.servicePartnerName || '').toLowerCase().includes(term) ||
      (j.externalVendorName || '').toLowerCase().includes(term) ||
    ...
    // Status filter
    if (statusFilter !== 'All') {
      if (statusFilter === 'External Repair') {
        const isExt = j.repairType === 'External Service Partner' || j.repairType === 'EXTERNAL_PARTNER' || !!j.servicePartnerId || !!j.servicePartnerName || !!j.externalVendorName;
        if (!isExt) return false;
      } else if (statusFilter === 'Device Received') {
        if (j.status !== 'Device Received' && j.status !== 'Received') return false;
      } else if (statusFilter === 'Work in Progress') {
        if (j.status !== 'Work in Progress' && j.status !== 'Pending') return false;
      } else if (statusFilter === 'Approval Pending') {
        if (j.status !== 'Approval Pending') return false;
      } else if (statusFilter === 'Device Ready') {
        if (j.status !== 'Device Ready' && j.status !== 'Ready' && j.status !== 'Completed' && j.status !== 'Complete & Ready') return false;
      } else if (statusFilter === 'Device Not repairable') {
        if (j.status !== 'Device Not repairable' && j.status !== 'Not Repaired' && j.repairOutcome !== 'Not Repaired') return false;
    ...
    // Date range filter
    const jobDate = j.date ? j.date.substring(0, 10) : (j.createdAt ? j.createdAt.substring(0, 10) : '');
    ...
    if (dateFilter === 'today') {
      ...
      const weekStartStr = firstDayOfWeek.toISOString().substring(0, 10);
      if (jobDate < weekStartStr) return false;
    } else if (dateFilter === 'this_month') {
      const monthStartStr = today.substring(0, 7) + '-01';
      if (jobDate < monthStartStr) return false;
  ...
  const handleOpenNewJob = () => {
    ...
    setServicePartnerCost(0);
    setPartnerSentDate(new Date().toISOString().substring(0, 10));
    setPartnerExpectedReturnDate('');
    setPartnerChallanNumber('');
    setPartnerStatus('Sent to Service Partner');
    setPartnerRemarks('');
    setPartnerWorkDescription('');
  ...
  const handleSaveInlineProblem = async () => {
    const trimmed = inlineProblemName.trim();
    if (!trimmed) return;
    if (onAddProblem) {
      await onAddProblem(trimmed);
    }
    const upper = trimmed.toUpperCase();
    if (!selectedProblems.includes(upper)) {
      setSelectedProblems(prev => [...prev, upper]);
    }
    setInlineProblemName('');
    setShowInlineAddProblem(false);
  };

  const handleSaveQuickPartner = () => {
    const name = quickPartnerName.trim();
    if (!name) {
      alert('Please enter Service Partner Name.');
      return;
    }
    if (onAddServicePartner) {
      onAddServicePartner({
        name,
        contactPerson: name,
        mobile: quickPartnerMobile.trim(),
        phone: quickPartnerMobile.trim(),
        city: quickPartnerCity.trim() || 'Local',
        state: 'Maharashtra',
        address: '',
        email: '',
        specializations: ['Chip Level Repair', 'BGA Rework', 'Panel Repair'],
        status: 'Active',
        activeJobsCount: 0,
        completedJobsCount: 0,
        totalBilled: 0
      });
      setServicePartnerName(name);
    }
    setQuickPartnerName('');
    setQuickPartnerMobile('');
    setQuickPartnerCity('');
  ...
  const handleSaveNewJob = (keepOpenAndAddAnother: boolean = false) => {
    ...
    const resolvedPartnerName = servicePartnerId ? (servicePartners.find(p => p.id === servicePartnerId)?.name || servicePartnerName) : servicePartnerName;
    ...
    onAddJob({
      ...
      repairType: repairType || 'Internal',
      servicePartnerId: repairType === 'External Service Partner' ? servicePartnerId : undefined,
      servicePartnerName: repairType === 'External Service Partner' ? resolvedPartnerName : undefined,
      externalVendorName: repairType === 'External Service Partner' ? resolvedPartnerName : undefined,
      servicePartnerCost: repairType === 'External Service Partner' ? (Number(servicePartnerCost) || 0) : undefined,
      partnerEstimatedCost: repairType === 'External Service Partner' ? (Number(servicePartnerCost) || 0) : undefined,
      partnerSentDate: repairType === 'External Service Partner' ? (partnerSentDate || dateStr) : undefined,
      partnerExpectedReturnDate: repairType === 'External Service Partner' ? partnerExpectedReturnDate : undefined,
      partnerChallanNumber: repairType === 'External Service Partner' ? partnerChallanNumber : undefined,
      partnerStatus: repairType === 'External Service Partner' ? (partnerStatus as any || 'Sent to Service Partner') : undefined,
      partnerWorkDescription: repairType === 'External Service Partner' ? partnerWorkDescription : undefined,
      partnerRemarks: repairType === 'External Service Partner' ? partnerRemarks : undefined
    });
  ...
  const handleOpenManageJob = (job: RepairJob) => {
    ...
    // External partner assignment
    const isExt = job.repairType === 'External Service Partner' || job.repairType === 'EXTERNAL_PARTNER' || !!job.servicePartnerId || !!job.servicePartnerName || !!job.externalVendorName;
    setRepairType(isExt ? 'External Service Partner' : 'Internal');
    setServicePartnerId(job.servicePartnerId || '');
    setServicePartnerName(job.servicePartnerName || job.externalVendorName || '');
    setServicePartnerCost(job.servicePartnerCost || job.partnerEstimatedCost || 0);
    setPartnerSentDate(job.partnerSentDate || job.date || '');
    setPartnerExpectedReturnDate(job.partnerExpectedReturnDate || '');
    setPartnerChallanNumber(job.partnerChallanNumber || '');
    setPartnerStatus((job.partnerStatus as string) || 'Sent to Service Partner');
    setPartnerRemarks(job.partnerRemarks || '');
    setPartnerWorkDescription(job.partnerWorkDescription || '');
  ...
  const handleSaveManagedJob = (sendWhatsApp: boolean = false) => {
    ...
    const updatedJob: RepairJob = {
      ...
      servicePartnerCost: repairType === 'External Service Partner' ? (Number(servicePartnerCost) || 0) : undefined,
      partnerEstimatedCost: repairType === 'External Service Partner' ? (Number(servicePartnerCost) || 0) : undefined,
      partnerSentDate: repairType === 'External Service Partner' ? partnerSentDate : undefined,
      partnerExpectedReturnDate: repairType === 'External Service Partner' ? partnerExpectedReturnDate : undefined,
      partnerChallanNumber: repairType === 'External Service Partner' ? partnerChallanNumber : undefined,
      partnerStatus: repairType === 'External Service Partner' ? (partnerStatus as any || 'Sent to Service Partner') : undefined,
      partnerWorkDescription: repairType === 'External Service Partner' ? partnerWorkDescription : undefined,
  ...
  const getInwardStatusBorderStyles = (status: string, repairOutcome?: string) => {
    const isNotRepaired = status === 'Device Not repairable' || status === 'Not Repaired' || repairOutcome === 'Not Repaired';
    if (isNotRepaired) {
      return {
        rowClass: 'border-l-4 border-l-rose-500 border-b border-b-rose-100 hover:bg-rose-50/50 bg-rose-50/15 transition',
        cardClass: 'border-2 border-l-4 border-rose-300 border-l-rose-600 bg-rose-50/20 rounded-2xl shadow-2xs'
      };
    }
    if (status === 'Received' || status === 'Device Received') {
      return {
        rowClass: 'border-l-4 border-l-blue-500 border-b border-b-blue-100 hover:bg-blue-50/50 bg-blue-50/15 transition',
        cardClass: 'border-2 border-l-4 border-blue-300 border-l-blue-600 bg-blue-50/20 rounded-2xl shadow-2xs'
      };
    }
    if (status === 'Work in Progress' || status === 'Pending') {
      return {
        rowClass: 'border-l-4 border-l-amber-500 border-b border-b-amber-100 hover:bg-amber-50/50 bg-amber-50/15 transition',
        cardClass: 'border-2 border-l-4 border-amber-300 border-l-amber-600 bg-amber-50/20 rounded-2xl shadow-2xs'
      };
    }
    if (status === 'Approval Pending') {
      return {
        rowClass: 'border-l-4 border-l-orange-500 border-b border-b-orange-100 hover:bg-orange-50/50 bg-orange-50/15 transition',
        cardClass: 'border-2 border-l-4 border-orange-300 border-l-orange-600 bg-orange-50/20 rounded-2xl shadow-2xs'
      };
    }
    if (status === 'Ready' || status === 'Complete & Ready' || status === 'Completed' || status === 'Device Ready') {
      return {
        rowClass: 'border-l-4 border-l-purple-500 border-b border-b-purple-100 hover:bg-purple-50/50 bg-purple-50/15 transition',
        cardClass: 'border-2 border-l-4 border-purple-300 border-l-purple-600 bg-purple-50/20 rounded-2xl shadow-2xs'
      };
    }
    if (status === 'Product Out' || status === 'Outwarded' || status === 'Delivered') {
      return {
        rowClass: 'border-l-4 border-l-emerald-500 border-b border-b-emerald-100 hover:bg-emerald-50/50 bg-emerald-50/15 transition',
        cardClass: 'border-2 border-l-4 border-emerald-300 border-l-emerald-600 bg-emerald-50/20 rounded-2xl shadow-2xs'
      };
    }
    return {
      rowClass: 'border-l-4 border-l-teal-500 border-b border-b-teal-100 hover:bg-teal-50/50 bg-teal-50/15 transition',
      cardClass: 'border-2 border-l-4 border-teal-300 border-l-teal-600 bg-teal-50/20 rounded-2xl shadow-2xs'
    };
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50/60 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          <div className="relative flex-1">
            <Search className="absolute left-3.5 top-2.5 w-4.5 h-4.5 text-slate-400" />
            <input
              type="text"
              placeholder="Search by Job ID, Client, Device Name, Serial, Partner or Tech..."
              value={searchTerm}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl px-2.5 py-1 text-xs">
              ...
              >
                <option value="All">All Statuses ({inwardJobs.length})</option>
                ...
                <option value="External Repair">External Repair / Partners</option>
              </select>
            </div>
            ...
            {dateFilter === 'custom' && (
              <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl px-2 py-1 text-xs">
                <input
                  ...
                  className="text-[11px] text-slate-700 font-medium focus:outline-none"
                  title="From Date"
                />
            ...
            {/* Reset Filters */}
            {(searchTerm || statusFilter !== 'All' || dateFilter !== 'all' || fromDate || toDate) && (
              <button
                type="button"
                onClick={() => {
                  setSearchTerm('');
                ...
                className="px-2.5 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 rounded-xl transition flex items-center gap-1 cursor-pointer"
        ...
        <div className="md:hidden p-3 space-y-3 bg-slate-50/50">
          {filteredJobs.length > 0 ? (
            filteredJobs.map((job) => {
              const borderStyles = getInwardStatusBorderStyles(job.status, job.repairOutcome);
              return (
              <div key={job.id} className={`p-4 space-y-3 bg-white ${borderStyles.cardClass}`}>
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {(job.repairType === 'External Service Partner' || job.servicePartnerName || job.externalVendorName) && (
                      <span className="inline-flex items-center gap-1 text-[9px] bg-amber-100 text-amber-800 border border-amber-300 px-1.5 py-0.5 rounded-full font-bold">
                        <Building className="w-2.5 h-2.5" />
                        {job.servicePartnerName || job.externalVendorName || 'Partner'}
                      </span>
                    )}
                  </div>
                </div>
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {filteredJobs.length > 0 ? (
                filteredJobs.map((job) => {
                  const borderStyles = getInwardStatusBorderStyles(job.status, job.repairOutcome);
                  return (
                  <tr key={job.id} className={`${borderStyles.rowClass}`}>
                    ...
                    <td className="py-3.5 px-4">
                      <div className="space-y-1">
                        ...
                        {(job.repairType === 'External Service Partner' || job.servicePartnerName || job.externalVendorName) && (
                          <div>
                            <span className="inline-flex items-center gap-1 text-[9px] bg-amber-50 text-amber-800 border border-amber-200 px-1.5 py-0.5 rounded font-bold" title={job.servicePartnerName || job.externalVendorName}>
                              <Building className="w-2.5 h-2.5 text-amber-600" />
                              <span className="max-w-[110px] truncate">{job.servicePartnerName || job.externalVendorName || 'Partner'}</span>
                            </span>
                          </div>
                        )}
                      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/80 space-y-2.5">
                    ...
                    {/* Repair Type Selector: Internal vs External Service Partner */}
                    <div className="space-y-1">
                      <label className="block font-bold text-slate-600 uppercase text-[10px]">Repair Execution Mode</label>
                      <div className="grid grid-cols-2 gap-1.5">
                        <button
                          type="button"
                          onClick={() => setRepairType('Internal')}
                          className={`py-1.5 px-2 rounded-lg text-xs font-bold border transition flex items-center justify-center gap-1 cursor-pointer ${
                            repairType === 'Internal'
                              ? 'bg-teal-600 text-white border-teal-600 shadow-xs'
                        ...
                        >
                          ...
                          <span>Internal In-House</span>
                        </button>
                        <button
                          type="button"
                          onClick={() => setRepairType('External Service Partner')}
                          className={`py-1.5 px-2 rounded-lg text-xs font-bold border transition flex items-center justify-center gap-1 cursor-pointer ${
                            repairType === 'External Service Partner'
                    ...
                    {/* If External Service Partner is selected */}
                    {repairType === 'External Service Partner' && (
                      <div className="p-2.5 bg-amber-50/80 border border-amber-200 rounded-xl space-y-2 animate-fade-in">
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] font-bold text-amber-900 flex items-center gap-1">
                            <Building className="w-3.5 h-3.5 text-amber-700" />
                            External Service Partner Details
                          </span>
                          <button
                            type="button"
                            onClick={() => setShowQuickAddPartner(true)}
                            className="text-[10px] font-bold text-amber-800 bg-amber-100 hover:bg-amber-200 border border-amber-300 px-2 py-0.5 rounded-lg flex items-center gap-1 transition cursor-pointer"
                          >
                            <Plus className="w-3 h-3" /> Quick Add Partner
                          </button>
                        </div>

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <div className="space-y-1 sm:col-span-2">
                            <label className="block font-bold text-amber-900 uppercase text-[10px]">Select Partner *</label>
                            <select
                              value={servicePartnerId}
                              onChange={(e) => {
                                const pId = e.target.value;
                                setServicePartnerId(pId);
                                const found = servicePartners.find(p => p.id === pId);
                                if (found) setServicePartnerName(found.name);
                              }}
                              className="w-full bg-white border border-amber-300 rounded-lg px-2 py-1 text-xs font-bold text-slate-800 focus:ring-1 focus:ring-amber-500"
                            >
                              <option value="">-- Choose Outsource Service Partner --</option>
                              {servicePartners.map(p => (
                                <option key={p.id} value={p.id}>{p.name} ({p.city || 'Vendor'})</option>
                              ))}
                            </select>
                          </div>

                          <div className="space-y-1">
                            <label className="block font-bold text-amber-900 uppercase text-[10px]">Partner Cost / Estimate (₹)</label>
                            <input
                              type="number"
                              min={0}
                              placeholder="0.00"
                              value={servicePartnerCost === 0 ? '' : servicePartnerCost}
                              onChange={(e) => setServicePartnerCost(e.target.value === '' ? 0 : Number(e.target.value))}
                              className="w-full bg-white border border-amber-300 rounded-lg px-2 py-1 text-xs font-mono font-bold text-slate-900 focus:ring-1 focus:ring-amber-500"
                            />
                          </div>

                          <div className="space-y-1">
                            <label className="block font-bold text-amber-900 uppercase text-[10px]">Challan / Gate Pass #</label>
                            <input
                              type="text"
                              placeholder="e.g. CH-2026-081"
                              value={partnerChallanNumber}
                              onChange={(e) => setPartnerChallanNumber(e.target.value)}
                              className="w-full bg-white border border-amber-300 rounded-lg px-2 py-1 text-xs font-mono text-slate-900 focus:ring-1 focus:ring-amber-500"
                            />
                          </div>

                          <div className="space-y-1">
                            <label className="block font-bold text-amber-900 uppercase text-[10px]">Sent Date</label>
                            <input
                              type="date"
                              value={partnerSentDate}
                              onChange={(e) => setPartnerSentDate(e.target.value)}
                              className="w-full bg-white border border-amber-300 rounded-lg px-2 py-1 text-xs text-slate-900 focus:ring-1 focus:ring-amber-500"
                            />
                          </div>

                          <div className="space-y-1">
                            <label className="block font-bold text-amber-900 uppercase text-[10px]">Expected Return Date</label>
                            <input
                              type="date"
                              value={partnerExpectedReturnDate}
                              onChange={(e) => setPartnerExpectedReturnDate(e.target.value)}
                              className="w-full bg-white border border-amber-300 rounded-lg px-2 py-1 text-xs text-slate-900 focus:ring-1 focus:ring-amber-500"
                            />
                          </div>

                          <div className="space-y-1 sm:col-span-2">
                            <label className="block font-bold text-amber-900 uppercase text-[10px]">Partner Status</label>
                            <select
                              value={partnerStatus}
                              onChange={(e) => setPartnerStatus(e.target.value)}
                              className="w-full bg-white border border-amber-300 rounded-lg px-2 py-1 text-xs font-bold text-amber-900"
                            >
                              <option value="Sent to Service Partner">Sent to Service Partner</option>
                              <option value="In Diagnostics with Partner">In Diagnostics with Partner</option>
                              <option value="Repaired by Partner">Repaired by Partner</option>
                              <option value="Returned from Partner">Returned from Partner</option>
                              <option value="Unrepairable by Partner">Unrepairable by Partner</option>
                            </select>
                          </div>

                          <div className="space-y-1 sm:col-span-2">
                            <label className="block font-bold text-amber-900 uppercase text-[10px]">Partner Work Scope / Instructions</label>
                            <input
                              type="text"
                              placeholder="e.g. BGA Reballing / Power IC replacement required..."
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <label className="block font-bold text-slate-600 uppercase text-[10px]">Client Estimate (₹)</label>
                        <input
                          type="number"
                          min={0}
                          placeholder="0.00"
                      ...
                      <div className="space-y-1">
                        <label className="block font-bold text-slate-600 uppercase text-[10px]">Assigned Internal Lead</label>
                        <select
                  ...
                  <div className="bg-slate-50/70 p-3 rounded-xl border border-slate-200/80 space-y-2.5">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          type="button"
                          onClick={() => setShowInlineAddProblem(!showInlineAddProblem)}
                          className="text-[10px] font-bold text-teal-700 bg-teal-50 hover:bg-teal-100 border border-teal-200 px-2 py-0.5 rounded-lg flex items-center gap-1 transition cursor-pointer"
                        >
                          <Plus className="w-3 h-3" /> Add Fault
                        </button>
                      </div>

                      {/* Inline Add Fault Input */}
                      {showInlineAddProblem && (
                        <div className="p-2 bg-teal-50/80 border border-teal-200 rounded-xl space-y-1.5 animate-fade-in">
                          <span className="text-[10px] font-bold text-teal-900 block">Create & Select New Fault Tag:</span>
                          <div className="flex gap-1.5">
                            <input
                              type="text"
                              placeholder="e.g. BIOS CORRUPT / NO DISPLAY"
                              value={inlineProblemName}
                              onChange={(e) => setInlineProblemName(e.target.value)}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  e.preventDefault();
                                  handleSaveInlineProblem();
                                }
                              }}
                              className="flex-1 bg-white border border-teal-300 rounded-lg px-2.5 py-1 text-xs font-semibold uppercase focus:outline-none focus:ring-1 focus:ring-teal-500"
                            />
                            <button
                              type="button"
                              onClick={handleSaveInlineProblem}
                              className="px-3 py-1 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-lg text-xs transition cursor-pointer shadow-xs"
                            >
                              Save Tag
                            </button>
                            <button
                              ...
                              className="px-2 py-1 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-lg text-xs transition cursor-pointer"
                            >
                              Cancel
                            </button>
                          </div>
      ...
      {/* Quick Add Service Partner Modal */}
      {showQuickAddPartner && (
        <div 
        ...
        >
          <div 
            className="bg-white rounded-2xl max-w-md w-full p-5 shadow-2xl space-y-4 animate-scale-up"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-slate-800 text-sm">Quick Add Service Partner</h3>
                  <p className="text-[10px] text-slate-400">Add external repair lab / outsource vendor</p>
                </div>
              </div>
              <button 
                onClick={() => setShowQuickAddPartner(false)}
            ...
            <div className="space-y-3 text-xs">
              <div className="space-y-1">
                <label className="block font-bold text-slate-600 uppercase text-[10px]">Partner / Lab Name *</label>
                <input
                  type="text"
                  placeholder="e.g. Apex Chip-Level Solutions"
                  value={quickPartnerName}
                  onChange={(e) => setQuickPartnerName(e.target.value)}
                  className="w-full border border-slate-300 rounded-xl px-3 py-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-amber-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="block font-bold text-slate-600 uppercase text-[10px]">Phone / Mobile</label>
                  <input
                    type="tel"
                    placeholder="e.g. 9876543210"
                    value={quickPartnerMobile}
                    onChange={(e) => setQuickPartnerMobile(e.target.value)}
                    className="w-full border border-slate-300 rounded-xl px-3 py-2 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-amber-500"
                  />
                </div>

                <div className="space-y-1">
                  <label className="block font-bold text-slate-600 uppercase text-[10px]">City / Location</label>
                  <input
                    type="text"
                    placeholder="e.g. Lamington Rd, Mumbai"
                    value={quickPartnerCity}
                    onChange={(e) => setQuickPartnerCity(e.target.value)}
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl text-xs font-bold bg-amber-600 hover:bg-amber-700 text-white shadow-xs transition"
              >
                Save Partner
              </button>
            </div>
