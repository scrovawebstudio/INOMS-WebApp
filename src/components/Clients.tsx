    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden">
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {isAdmin ? (
            /* ADMIN VIEW: CLIENT ORGANIZATIONS MOBILE CARDS */
            filteredTenants.length > 0 ? (
              filteredTenants.map((tenant) => (
                <div key={tenant.id} className="p-4 space-y-3 bg-white">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-8 h-8 rounded-xl bg-teal-50 text-teal-700 flex items-center justify-center shrink-0 font-bold border border-teal-200">
                        <Building className="w-4 h-4" />
                      </div>
                      <div className="min-w-0">
                        <h3 className="font-bold text-slate-900 text-sm truncate">{tenant.name}</h3>
                        <span className="font-mono text-xs font-bold text-teal-700">{tenant.code}</span>
                      </div>
                    </div>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold shrink-0 ${
                      tenant.status === 'active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'
                    }`}>
                  ...
                  <div className="text-xs text-slate-600 space-y-1 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-slate-700">{tenant.ownerName || 'Admin'}</span>
                    </div>
                    {tenant.ownerMobile && (
                      <div className="flex items-center justify-between">
                        <span className="text-slate-400">Mobile:</span>
                        <a href={`tel:${tenant.ownerMobile}`} className="font-mono font-bold text-teal-700 hover:underline">
                          {tenant.ownerMobile}
                        </a>
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className="flex-1 justify-center px-3 py-2 bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs rounded-xl transition cursor-pointer flex items-center gap-1.5 shadow-xs min-h-[38px]"
                    >
                    ...
                    {onToggleTenantStatus && tenant.id !== 'org-admin' && (
                      <button
                        ...
                        className={`px-3 py-2 rounded-xl font-bold text-xs transition cursor-pointer flex items-center gap-1 min-h-[38px] ${
              ...
              <div className="p-8 text-center text-slate-400 text-xs">No client organizations found.</div>
            )
          ) : (
            /* STORE CLIENTS MOBILE CARDS */
            filteredClients.length > 0 ? (
                ...
                return (
                  <div key={client.id} className="p-4 space-y-3 bg-white">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="font-bold text-slate-900 text-sm text-left hover:text-teal-700 transition"
                          >
                            {client.name}
                          </button>
                        ...
                        {client.mobile && (
                          <div className="mt-1">
                            <a
                              ...
                              className="text-xs font-mono font-bold text-teal-700 hover:underline inline-flex items-center gap-1"
                            >
                      ...
                      {/* Outstanding ledger balance badge */}
                      <div className="text-right shrink-0">
                        {bal > 0 ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-rose-50 border border-rose-200 text-rose-700 text-xs font-mono font-black">
                            <span>₹{Math.abs(bal).toFixed(2)}</span>
                            <span className="bg-rose-600 text-white text-[8px] font-black px-1 py-0.2 rounded uppercase">Dr</span>
                          </span>
                        ) : bal < 0 ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-mono font-black">
                            <span>₹{Math.abs(bal).toFixed(2)}</span>
                            <span className="bg-emerald-600 text-white text-[8px] font-black px-1 py-0.2 rounded uppercase">Cr</span>
                          </span>
                        ) : (
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-100">
                      <button
                        type="button"
                        onClick={() => handleOpenLedger(client)}
                        className="flex-1 justify-center flex items-center gap-1.5 px-3 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold text-xs rounded-xl transition cursor-pointer min-h-[38px]"
                      >
                      ...
                      <div className="flex items-center gap-1.5">
                        {!isStaff && (
                          <button
                            ...
                            title="Edit Client"
                            className="p-2 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-100 transition cursor-pointer min-h-[38px] min-w-[38px] flex items-center justify-center"
                          >
                        ...
                        {!isStaff && (
                          <button
                            ...
                            title="Delete Client"
                            className="p-2 bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-100 transition cursor-pointer min-h-[38px] min-w-[38px] flex items-center justify-center"
                          >
              ...
              <div className="p-8 text-center text-slate-400 text-xs">No clients found matching query.</div>
            )
          )}
        </div>
