...
import { LocalServerHubModal } from './LocalServerHubModal';
...
interface SettingsProps {
  ...
  appData?: {
    ...
    service_partner_payments?: any[];
    inventorySerials?: any[];
    inventory_serials?: any[];
...
export default function SettingsComponent({
  ...
  const applyImportedData = async (res: any) => {
    ...
    if (col) {
      ...
      const mySuppliers = getTenantRecords(col.suppliers, currentTenantId);
      const myServicePartners = getTenantRecords(col.servicePartners || col.service_partners, currentTenantId);
      const myPurchases = getTenantRecords(col.purchases, currentTenantId);
      const myPurchaseOrders = getTenantRecords(col.purchaseOrders || col.purchase_orders, currentTenantId);
      const myPurchaseReturns = getTenantRecords(col.purchaseReturns || col.purchase_returns, currentTenantId);
      const mySupplierPayments = getTenantRecords(col.supplierPayments || col.supplier_payments, currentTenantId);
      const myServicePartnerPayments = getTenantRecords(col.servicePartnerPayments || col.service_partner_payments, currentTenantId);
      const myInventorySerials = getTenantRecords(col.inventorySerials || col.inventory_serials, currentTenantId);
      const myInventoryTransactions = getTenantRecords(col.inventoryTransactions || col.inventory_transactions, currentTenantId);
      ...
      if (mySuppliers.length > 0) setAppStorageItem(`suppliers_${currentTenantId}`, JSON.stringify(mySuppliers));
      if (myServicePartners.length > 0) {
        setAppStorageItem(`servicePartners_${currentTenantId}`, JSON.stringify(myServicePartners));
        setAppStorageItem(`service_partners_${currentTenantId}`, JSON.stringify(myServicePartners));
      }
      if (myPurchases.length > 0) setAppStorageItem(`purchases_${currentTenantId}`, JSON.stringify(myPurchases));
      if (myPurchaseOrders.length > 0) {
        setAppStorageItem(`purchaseOrders_${currentTenantId}`, JSON.stringify(myPurchaseOrders));
        setAppStorageItem(`purchase_orders_${currentTenantId}`, JSON.stringify(myPurchaseOrders));
      }
      if (myPurchaseReturns.length > 0) {
        setAppStorageItem(`purchaseReturns_${currentTenantId}`, JSON.stringify(myPurchaseReturns));
        setAppStorageItem(`purchase_returns_${currentTenantId}`, JSON.stringify(myPurchaseReturns));
      }
      if (mySupplierPayments.length > 0) {
        setAppStorageItem(`supplierPayments_${currentTenantId}`, JSON.stringify(mySupplierPayments));
        setAppStorageItem(`supplier_payments_${currentTenantId}`, JSON.stringify(mySupplierPayments));
      }
      if (myServicePartnerPayments.length > 0) {
        setAppStorageItem(`servicePartnerPayments_${currentTenantId}`, JSON.stringify(myServicePartnerPayments));
        setAppStorageItem(`service_partner_payments_${currentTenantId}`, JSON.stringify(myServicePartnerPayments));
      }
      if (myInventorySerials.length > 0) {
        setAppStorageItem(`inventorySerials_${currentTenantId}`, JSON.stringify(myInventorySerials));
        setAppStorageItem(`inventory_serials_${currentTenantId}`, JSON.stringify(myInventorySerials));
      }
      if (myInventoryTransactions.length > 0) {
        setAppStorageItem(`inventoryTransactions_${currentTenantId}`, JSON.stringify(myInventoryTransactions));
        setAppStorageItem(`inventory_transactions_${currentTenantId}`, JSON.stringify(myInventoryTransactions));
      }
        ...
        try {
          ...
          const savePromises = tenantIds.map(async (tid: string) => {
            const tenantCollections: Record<string, any[]> = {
              ...
              suppliers: getTenantRecords(col.suppliers, tid),
              servicePartners: getTenantRecords(col.servicePartners || col.service_partners, tid),
              service_partners: getTenantRecords(col.servicePartners || col.service_partners, tid),
              purchases: getTenantRecords(col.purchases, tid),
              purchaseOrders: getTenantRecords(col.purchaseOrders || col.purchase_orders, tid),
              purchase_orders: getTenantRecords(col.purchaseOrders || col.purchase_orders, tid),
              purchaseReturns: getTenantRecords(col.purchaseReturns || col.purchase_returns, tid),
              purchase_returns: getTenantRecords(col.purchaseReturns || col.purchase_returns, tid),
              supplierPayments: getTenantRecords(col.supplierPayments || col.supplier_payments, tid),
              supplier_payments: getTenantRecords(col.supplierPayments || col.supplier_payments, tid),
              servicePartnerPayments: getTenantRecords(col.servicePartnerPayments || col.service_partner_payments, tid),
              service_partner_payments: getTenantRecords(col.servicePartnerPayments || col.service_partner_payments, tid),
              inventorySerials: getTenantRecords(col.inventorySerials || col.inventory_serials, tid),
              inventory_serials: getTenantRecords(col.inventorySerials || col.inventory_serials, tid),
              inventoryTransactions: getTenantRecords(col.inventoryTransactions || col.inventory_transactions, tid),
              inventory_transactions: getTenantRecords(col.inventoryTransactions || col.inventory_transactions, tid)
            };
  ...
  const restoreLocalBackup = async (backup: { name: string; data: any }) => {
    ...
    const collections = [
      ...
      'suppliers', 'servicePartners', 'service_partners', 'purchases', 'purchaseOrders', 'purchase_orders', 'purchaseReturns', 'purchase_returns',
      'supplierPayments', 'supplier_payments', 'servicePartnerPayments', 'service_partner_payments', 'inventorySerials', 'inventory_serials',
  ...
  const getCompiledTenantExportData = () => {
    return appData ? {
      ...
      suppliers: JSON.parse(getAppStorageItem(`suppliers_${currentTenantId}`) || '[]'),
      servicePartners: JSON.parse(getAppStorageItem(`servicePartners_${currentTenantId}`) || getAppStorageItem(`service_partners_${currentTenantId}`) || '[]'),
      service_partners: JSON.parse(getAppStorageItem(`service_partners_${currentTenantId}`) || getAppStorageItem(`servicePartners_${currentTenantId}`) || '[]'),
      purchases: JSON.parse(getAppStorageItem(`purchases_${currentTenantId}`) || '[]'),
      purchaseOrders: JSON.parse(getAppStorageItem(`purchaseOrders_${currentTenantId}`) || getAppStorageItem(`purchase_orders_${currentTenantId}`) || '[]'),
      purchase_orders: JSON.parse(getAppStorageItem(`purchase_orders_${currentTenantId}`) || getAppStorageItem(`purchaseOrders_${currentTenantId}`) || '[]'),
      purchaseReturns: JSON.parse(getAppStorageItem(`purchaseReturns_${currentTenantId}`) || getAppStorageItem(`purchase_returns_${currentTenantId}`) || '[]'),
      purchase_returns: JSON.parse(getAppStorageItem(`purchase_returns_${currentTenantId}`) || getAppStorageItem(`purchaseReturns_${currentTenantId}`) || '[]'),
      supplierPayments: JSON.parse(getAppStorageItem(`supplierPayments_${currentTenantId}`) || getAppStorageItem(`supplier_payments_${currentTenantId}`) || '[]'),
      supplier_payments: JSON.parse(getAppStorageItem(`supplier_payments_${currentTenantId}`) || getAppStorageItem(`supplierPayments_${currentTenantId}`) || '[]'),
      servicePartnerPayments: JSON.parse(getAppStorageItem(`servicePartnerPayments_${currentTenantId}`) || getAppStorageItem(`service_partner_payments_${currentTenantId}`) || '[]'),
      service_partner_payments: JSON.parse(getAppStorageItem(`service_partner_payments_${currentTenantId}`) || getAppStorageItem(`servicePartnerPayments_${currentTenantId}`) || '[]'),
      inventorySerials: JSON.parse(getAppStorageItem(`inventorySerials_${currentTenantId}`) || getAppStorageItem(`inventory_serials_${currentTenantId}`) || '[]'),
      inventory_serials: JSON.parse(getAppStorageItem(`inventory_serials_${currentTenantId}`) || getAppStorageItem(`inventorySerials_${currentTenantId}`) || '[]'),
      inventoryTransactions: JSON.parse(getAppStorageItem(`inventoryTransactions_${currentTenantId}`) || getAppStorageItem(`inventory_transactions_${currentTenantId}`) || '[]'),
      inventory_transactions: JSON.parse(getAppStorageItem(`inventory_transactions_${currentTenantId}`) || getAppStorageItem(`inventoryTransactions_${currentTenantId}`) || '[]')
    };
  };
      ...
      try {
        ...
        if (wb.SheetNames.includes('Inward & Outward Repair Jobs') || wb.SheetNames.includes('Jobs')) {
          ...
          restoredData.jobs = rawJobs.map(j => ({
            ...
            assignedTechnician: j['Assigned Technician'] || j['assignedTechnician'] || '',
    ...
    <div className="space-y-6 relative pb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 pt-2">
                ...
                {/* Social Media, Reviews & Custom URLs Section */}
                <div className="pt-4 border-t border-slate-100 space-y-3">
                  <div>
                    <h3 className="font-bold text-slate-800 text-sm mb-0.5 flex items-center gap-2">
                      <Share2 className="w-4 h-4 text-teal-600" />
                      Social Media, Google Review & Custom Links
                    </h3>
                    <p className="text-slate-400 text-[11px]">
                      Add your review links & social handles. These automatically attach to WhatsApp customer job updates so clients can review and follow!
                    </p>
                  </div>
                  ...
                  <div className="space-y-2.5">
                    ...
                    <div className="space-y-1">
                      <label className="flex items-center justify-between text-[10px] font-bold text-slate-600 uppercase">
                        <span className="flex items-center gap-1.5 text-amber-600">
                          <Star className="w-3.5 h-3.5 fill-amber-500" /> Google Business Review URL
                        </span>
                        <span className="text-slate-400 font-normal lowercase text-[10px]">e.g. https://g.page/r/your-id/review</span>
                      </label>
                      <input
                        type="url"
                        placeholder="https://g.page/r/.../review"
                        value={localConfig.googleReviewUrl || ''}
                        onChange={(e) => updateLocalConfig({ googleReviewUrl: e.target.value })}
                        className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono text-slate-800"
                      />
                    </div>

                    {/* WhatsApp Channel or Community */}
                    <div className="space-y-1">
                      <label className="flex items-center justify-between text-[10px] font-bold text-slate-600 uppercase">
                        <span className="flex items-center gap-1.5 text-emerald-600">
                          <MessageCircle className="w-3.5 h-3.5" /> WhatsApp Channel / Group URL
                        </span>
                        <span className="text-slate-400 font-normal lowercase text-[10px]">e.g. https://whatsapp.com/channel/...</span>
                      </label>
                      <input
                        type="url"
                        placeholder="https://whatsapp.com/channel/..."
                        value={localConfig.whatsappChannelUrl || ''}
                        onChange={(e) => updateLocalConfig({ whatsappChannelUrl: e.target.value })}
                        className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono text-slate-800"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <label className="block font-bold text-slate-500 uppercase text-[9px]">Instagram Profile URL</label>
                        <input
                          type="url"
                          placeholder="https://instagram.com/your-business"
                          value={localConfig.instagramUrl || ''}
                          onChange={(e) => updateLocalConfig({ instagramUrl: e.target.value })}
                          className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 font-mono text-slate-700 text-xs"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="block font-bold text-slate-500 uppercase text-[9px]">Facebook Page URL</label>
                        <input
                          type="url"
                          placeholder="https://facebook.com/your-page"
                          value={localConfig.facebookUrl || ''}
                          onChange={(e) => updateLocalConfig({ facebookUrl: e.target.value })}
                          className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 font-mono text-slate-700 text-xs"
                        />
                      </div>
                    </div>

                    {/* Custom Review / Website Link */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <div className="space-y-1">
                        <label className="block font-bold text-slate-500 uppercase text-[9px]">Custom Feedback / Review Link</label>
                        <input
                          type="url"
                          placeholder="https://yourdomain.com/feedback"
                          value={localConfig.customReviewUrl || ''}
                          onChange={(e) => updateLocalConfig({ customReviewUrl: e.target.value })}
                          className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 font-mono text-slate-700 text-xs"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="block font-bold text-slate-500 uppercase text-[9px]">YouTube / Portfolio URL</label>
                        <input
                          type="url"
                          placeholder="https://youtube.com/@yourchannel"
                          value={localConfig.youtubeUrl || ''}
                          onChange={(e) => updateLocalConfig({ youtubeUrl: e.target.value })}
                          className="w-full border border-slate-200 rounded-lg px-2.5 py-1.5 font-mono text-slate-700 text-xs"
                        />
                      </div>
                    </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-slate-100">
              ...
              <div className="flex flex-wrap items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-4 py-2.5 rounded-xl transition cursor-pointer text-xs flex items-center gap-2 shadow-xs"
                  title="Export complete organization data to Multi-Sheet Excel Spreadsheet (.xlsx)"
                >
                    ...
                    onClick={async () => {
                      ...
                      if (confirmPrompt === 'CLEAR') {
                        ...
                        const collectionsToPurge = [
                          'clients', 'jobs', 'invoices', 'products', 'ledger', 'payments', 'expenses',
                          'categories', 'racks', 'equipments', 'problems',
                          'suppliers', 'servicePartners', 'service_partners',
                          'purchases', 'purchaseOrders', 'purchase_orders',
            ...
            <div className="lg:col-span-1 bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">Manage, deactivate or delete Staff profiles created by your organization.</p>
                </div>
                {features.allowTechnicianAccounts && (
                  <button
                    type="button"
              ...
              <div className="space-y-2.5">
                {users.filter(u => {
                  // Only organization staff accounts should be displayed; exclude admin/master admin accounts
                  if (u.role === 'Admin') return false;
                  if (u.mobile?.includes('8149862034') || u.username === 'scrova' || u.username === 'inoms' || u.email === 'admin@mastersystem.com' || u.email === 'admin@inoms.com' || u.name === 'Master System Admin' || u.name === 'INOMS Admin') {
                    return false;
                  }
                  return true;
                }).map((u, uIdx) => {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="Staff">General Staff / Assistant</option>
                  </select>
                </div>
              </div>
      ...
      {activeSubTab === 'localserver' && (
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xs space-y-6" id="settings-localserver">
          ...
          {/* Capability check */}
          {!hasCapability(activeTenant, 'local_server') ? (
            <div className="p-5 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 space-y-3">
              ...
              <div className="flex items-center gap-3 pt-1">
                <span className="text-xs font-semibold text-amber-900">
                  Contact administrator or upgrade via the Billing portal to enable Pro capabilities.
                </span>
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              {/* Architecture Core Directives Cards */}
              ...
              {/* Action Callout */}
              <div className="p-5 rounded-2xl bg-gradient-to-br from-purple-900 to-indigo-950 text-white flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="space-y-1">
