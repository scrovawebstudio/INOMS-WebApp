...
import { AddonPricingConfig, MasterAdminInvoice, MasterAdminInvoiceItem, SubscriptionRenewalRequest } from '../types';
...
export default function MasterAdminBilling({
  ...
  const populateLineItemsForTenant = (tenantId: string, period: typeof billingPeriod) => {
    ...
    // 2. Barcode & QR Code Tagging Add-on
    if (features.allowBarcodeQrTags) {
      const addonRate = pricingConfig.featureAddons?.find(f => f.key === 'barcodeQrTags')?.monthlyPrice ?? pricingConfig.barcodeQrTags ?? 299;
      items.push({
        id: 'item-barcode-qr',
  ...
  const handleSendWhatsApp = (inv: MasterAdminInvoice) => {
    ...
    msg += `📲 *Pay via UPI:* sujitgajare@okicici\n`;
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-teal-950 to-slate-900 p-4 sm:p-6 rounded-3xl text-white shadow-xl flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="grid grid-cols-2 sm:flex items-center gap-2 sm:gap-3">
          ...
          <div className="bg-white/10 backdrop-blur-md px-3 sm:px-4 py-2 rounded-2xl border border-white/10 text-center">
            <span className="text-[9px] sm:text-[10px] font-bold text-slate-300 uppercase block">Pending Renewals</span>
            <span className={`text-base sm:text-lg font-black font-mono ${renewalRequests.filter(r => r.status === 'pending').length > 0 ? 'text-amber-400' : 'text-slate-300'}`}>
      ...
      {/* Pending Renewal Requests Section (Fast-Track Master Admin Approval) */}
      {renewalRequests && renewalRequests.length > 0 && (
        <div className="bg-gradient-to-br from-amber-500/10 via-teal-500/5 to-slate-50 border-2 border-amber-400/60 rounded-3xl p-4 sm:p-6 shadow-md space-y-4">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-sm sm:text-base font-extrabold text-slate-900 flex items-center gap-2">
                  <span>Direct UPI Subscription Renewal Requests</span>
                ...
                <p className="text-xs text-slate-600">
                  Organizations have submitted their UPI UTR reference codes. Verify the UTR in your bank statement and click <strong>"Approve & Activate"</strong> for instant 1-click renewal.
                </p>
              </div>
            </div>
          </div>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <h4 className="font-extrabold text-slate-900 text-sm mt-1">{req.tenantName}</h4>
                      <p className="text-xs text-slate-600">
                        {req.ownerName} &bull; <span className="font-mono">{req.ownerMobile}</span>
                      </p>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <span className={`inline-block text-[9px] font-extrabold uppercase px-2 py-0.5 rounded-full ${
                        isPending ? 'bg-amber-100 text-amber-800' : req.status === 'approved' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'
                      }`}>
                        {req.status}
                      </span>
                    </div>
                  ...
                  <div className="mt-3 bg-slate-50 p-2.5 rounded-xl border border-slate-200/70 text-xs space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 font-medium">Selected Plan:</span>
                      <span className="font-bold text-slate-800">{req.planTitle}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 font-medium">UPI UTR / Ref No:</span>
                    ...
                    <div className="flex items-center justify-between text-[10px] text-slate-400">
                      <span>Submitted:</span>
                      <span>{new Date(req.submittedAt).toLocaleString('en-IN')}</span>
                    </div>
                    {req.notes && (
                      <p className="text-[11px] text-slate-600 italic pt-1 border-t border-slate-200/50">
                        "{req.notes}"
                      </p>
                    )}
                  </div>
                  ...
                  {isPending && (
                    <div className="mt-3 flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => onApproveRenewal?.(req)}
                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs py-2 px-3 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5 shadow-sm"
                      >
                        <CheckCircle2 className="w-3.5 h-3.5" />
                        <span>Approve & Activate Workspace</span>
                      </button>
                      <button
                        type="button"
                        onClick={() => onRejectRenewal?.(req.id)}
                        className="text-xs font-bold text-rose-600 hover:bg-rose-50 px-3 py-2 rounded-xl transition cursor-pointer border border-rose-200"
                      >
                        Reject
                      </button>
                    </div>
                  ...
                  {req.status === 'approved' && (
                    <p className="mt-2 text-[10px] font-bold text-emerald-700 flex items-center gap-1">
                      <Check className="w-3.5 h-3.5" /> Activated on {req.approvedAt ? new Date(req.approvedAt).toLocaleDateString('en-IN') : 'Recently'}
                    </p>
                  )}
                </div>
              );
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                  <div className="flex items-center gap-3 bg-emerald-50/50 p-2.5 rounded-xl border border-emerald-100/50">
                    <div className="w-14 h-14 bg-white rounded-lg border border-emerald-200 flex items-center justify-center p-0.5 shrink-0 overflow-hidden">
                      <img
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(`upi://pay?pa=sujitgajare@okicici&pn=INOMS%20Cloud%20Platform&am=${previewInvoice.grandTotal}&cu=INR`)}`}
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[9.5px] text-emerald-700 font-bold font-mono">UPI ID: sujitgajare@okicici</p>
