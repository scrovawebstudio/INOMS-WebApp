...
export default function Payments({
  ...
  const [selectedMultiJobIds, setSelectedMultiJobIds] = useState<string[]>([]);
  const [paymentTargetType, setPaymentTargetType] = useState<'single' | 'multi'>('single');
  ...
  const handleOpenAdd = () => {
    ...
    setRemarks('');
    setLinkedRef('');
    setSelectedMultiJobIds([]);
    setPaymentTargetType('single');
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    let linkedJobIds: string[] | undefined = undefined;

    if (paymentTargetType === 'multi' && selectedMultiJobIds.length > 0) {
      linkedJobIds = selectedMultiJobIds;
    } else if (linkedRef.startsWith('job:')) {
      linkedJobId = linkedRef.replace('job:', '');
    } else if (linkedRef.startsWith('inv:')) {
      invoiceId = linkedRef.replace('inv:', '');
    }

    const refString = refNo || (
      linkedJobIds && linkedJobIds.length > 0
        ? `Lump-Sum (${linkedJobIds.length} Jobs: ${linkedJobIds.join(', ')})`
        : (linkedJobId ? `JOB-${linkedJobId}` : (invoiceId ? `INV-${invoiceId}` : undefined))
    );
    ...
    onAddPayment({
      ...
      mode,
      refNo: refString,
      remarks: remarks || (linkedJobIds?.length ? `Lump-sum payment clearing JCs: ${linkedJobIds.join(', ')}` : undefined),
      linkedJobId,
      linkedJobIds,
      invoiceId
    });
  ...
  return (
    <div className="space-y-6">
      ...
      {showAddPayment && (
        ...
        >
          <div 
            className="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-3xl w-[95vw] overflow-hidden animate-slide-up cursor-default my-auto max-h-[92vh] flex flex-col"
          ...
          >
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-900 text-white shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-sm font-bold text-white tracking-tight">Record Credit Payment / Settle Job Cards</h2>
                  <p className="text-[11px] text-slate-400">Receive payment & clear multiple job cards or invoices</p>
                </div>
              </div>
              <button 
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 text-xs overflow-y-auto flex-1">
              ...
              <div className="space-y-2 pt-2 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <label className="block font-bold text-slate-800 uppercase text-[10px] tracking-wider">
                    Allocation / Link to Job Cards & Invoices
                  </label>
                  <div className="flex items-center gap-1 bg-slate-100 border border-slate-200 p-1 rounded-xl text-[11px] font-bold">
                    <button
                      type="button"
                      onClick={() => {
                        setPaymentTargetType('multi');
                        setLinkedRef('');
                      }}
                      className={`px-3 py-1 rounded-lg transition cursor-pointer ${
                        paymentTargetType === 'multi'
                          ? 'bg-teal-600 text-white shadow-xs'
                          : 'text-slate-600 hover:text-slate-900'
                      }`}
                    >
                      Multiple Job Cards (Lump-Sum)
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className={`px-3 py-1 rounded-lg transition cursor-pointer ${
                        paymentTargetType === 'single'
                          ? 'bg-teal-600 text-white shadow-xs'
                          : 'text-slate-600 hover:text-slate-900'
                      }`}
                    >
                      Single Item / Tax Invoice
                    </button>
                  </div>
                </div>
                ...
                {paymentTargetType === 'single' ? (
                  <select
                    ...
                    className="w-full border border-slate-200 bg-white rounded-xl px-3.5 py-2 font-semibold text-slate-700 text-xs"
                  >
                    <option value="">General Client Payment (Unlinked / General Ledger Credit)</option>
                    {(() => {
                      const curClient = clients.find(c => c.id === clientId);
                      const isMatch = (item: { clientId?: string; clientMobile?: string; clientPhone?: string; clientName?: string }) => {
                        if (!item) return false;
                        if (item.clientId && item.clientId === clientId) return true;
                        if (curClient && item.clientMobile && curClient.mobile && item.clientMobile.replace(/\D/g, '') === curClient.mobile.replace(/\D/g, '')) return true;
                        if (curClient && item.clientName && curClient.name && item.clientName.trim().toLowerCase() === curClient.name.trim().toLowerCase()) return true;
                        return false;
                      };
                      const clientJobs = (jobs || []).filter(j => isMatch(j));
                      const clientInvs = (invoices || []).filter(inv => isMatch(inv));

                      return (
                        <>
                          <optgroup label="🛠️ Repair Job Cards">
                            {clientJobs.map(j => {
                              const isNotRepaired = j.status === 'Device Not repairable' || j.status === 'Not Repaired' || j.repairOutcome === 'Not Repaired';
                              const bill = isNotRepaired ? 0 : getEffectiveBillAmount(j);
                              const adv = Number(j.advanceAmount) || 0;
                              const pending = Math.max(0, bill - adv);
                              return (
                                <option key={`job:${j.id}`} value={`job:${j.id}`}>
                                  🛠️ Job Card #{j.id} - {j.productName || j.equipment} [Bill: ₹{bill} | Due: ₹{pending}] {isNotRepaired ? '(NOT REPAIRABLE)' : j.paymentStatus === 'Paid' ? '(PAID)' : '(UNPAID)'}
                                </option>
                              );
                            })}
                          </optgroup>
                          <optgroup label="🧾 Tax Invoices">
                            {clientInvs.map(inv => (
                              <option key={`inv:${inv.id}`} value={`inv:${inv.id}`}>
                                📄 Tax Invoice #{inv.id} (Total: ₹{inv.grandTotal} | Due: ₹{inv.balanceAmount ?? inv.grandTotal}) {inv.isPaid ? '(PAID)' : ''}
                              </option>
                            ))}
                          </optgroup>
                        </>
                  ...
                  <div className="space-y-2 bg-slate-50 p-3 rounded-2xl border border-slate-200">
                    {(() => {
                      ...
                      const clientJobs = (jobs || []).filter(j => isMatch(j));
                      const payableJobs = clientJobs.filter(j => j.status !== 'Device Not repairable' && j.status !== 'Not Repaired' && j.repairOutcome !== 'Not Repaired');

                      return (
                        <>
                          <div className="flex items-center justify-between text-xs text-slate-700 font-semibold">
                            <span>Select Job Cards to mark as Paid with this lump-sum payment:</span>
                            <div className="flex items-center gap-2">
                              {payableJobs.filter(j => j.paymentStatus !== 'Paid').length > 0 && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    const unpaidIds = payableJobs.filter(j => j.paymentStatus !== 'Paid').map(j => j.id);
                                    setSelectedMultiJobIds(unpaidIds);
                                    const totalDue = payableJobs.filter(j => unpaidIds.includes(j.id)).reduce((sum, j) => sum + Math.max(0, getEffectiveBillAmount(j) - (Number(j.advanceAmount) || 0)), 0);
                                    if (totalDue > 0) setAmount(totalDue);
                                  }}
                                  className="text-teal-700 bg-teal-50 hover:bg-teal-100 border border-teal-200 px-2 py-0.5 rounded text-[11px] font-bold cursor-pointer"
                                >
                                  Select All Unpaid
                                </button>
                              )}
                              {selectedMultiJobIds.length > 0 && (
                                <button
                                  type="button"
                                  onClick={() => setSelectedMultiJobIds([])}
                                  className="text-rose-600 hover:underline text-[11px] font-bold cursor-pointer"
                                >
                                  Clear Selection ({selectedMultiJobIds.length})
                                </button>
                              )}
                            </div>
                          </div>

                          <div className="max-h-52 overflow-y-auto space-y-1.5 bg-white p-2 rounded-xl border border-slate-200">
                            {clientJobs.length > 0 ? (
                              clientJobs.map(j => {
                                const isChecked = selectedMultiJobIds.includes(j.id);
                                const isNotRepaired = j.status === 'Device Not repairable' || j.status === 'Not Repaired' || j.repairOutcome === 'Not Repaired';
                                const bill = isNotRepaired ? 0 : getEffectiveBillAmount(j);
                                const adv = Number(j.advanceAmount) || 0;
                                const pending = Math.max(0, bill - adv);
                                  ...
                                  <label
                                    ...
                                    className={`flex items-center justify-between p-2.5 rounded-xl border text-xs cursor-pointer transition ${
                                      isChecked
                                        ...
                                        : isNotRepaired
                                        ? 'bg-amber-50/40 border-amber-200/60 text-slate-500'
                                  ...
                                  >
                                    <div className="flex items-center gap-3">
                                      <input
                                        type="checkbox"
                                        checked={isChecked}
                                        disabled={isNotRepaired}
                                        onChange={(e) => {
                                          if (e.target.checked) {
                                            setSelectedMultiJobIds(prev => [...prev, j.id]);
                                          } else {
                                            setSelectedMultiJobIds(prev => prev.filter(id => id !== j.id));
                                          }
                                        }}
                                      ...
                                      <div>
                                        <div className="flex items-center gap-1.5">
                                          <span className="font-mono font-bold text-slate-900 text-xs">#{j.id}</span>
                                          <span className="font-medium text-slate-800">{j.productName || j.equipment || 'Device'}</span>
                                          {j.productModel && <span className="text-[10px] text-slate-500 font-normal">({j.productModel})</span>}
                                        </div>
                                        <div className="text-[10px] text-slate-500 font-mono mt-0.5">
                                          Date: {j.date || (j.createdAt ? j.createdAt.split('T')[0] : '—')} • Status: {j.status || 'Received'}
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 text-[11px] font-mono shrink-0">
                                      <div className="text-right">
                                        <div className="text-slate-600">Bill: ₹{bill} {adv > 0 ? `| Adv: ₹${adv}` : ''}</div>
                                      </div>
                                      {isNotRepaired ? (
                                        ...
                                        <span className="bg-rose-100 text-rose-700 border border-rose-300 font-bold px-2 py-0.5 rounded text-[10px]">
                                          DUE: ₹{pending}
                                        </span>
                                      )}
                                    </div>
                              ...
                              <p className="text-xs text-slate-400 text-center py-4 italic">
                                No repair jobs found for this client.
                              </p>
                            )}
                          </div>

                          {selectedMultiJobIds.length > 0 && (
                            <div className="flex items-center justify-between text-xs bg-teal-100/70 border border-teal-200 px-3 py-2 rounded-xl text-teal-900 font-semibold">
                              <span><strong>{selectedMultiJobIds.length}</strong> Job Card(s) selected</span>
                              <button
                                type="button"
                                onClick={() => {
                                  const totalSelectedDue = payableJobs
                                    .filter(j => selectedMultiJobIds.includes(j.id))
                                    .reduce((sum, j) => sum + Math.max(0, getEffectiveBillAmount(j) - (Number(j.advanceAmount) || 0)), 0);
                                  if (totalSelectedDue > 0) {
                                    setAmount(totalSelectedDue);
                                  }
                                }}
                                className="text-xs text-teal-900 bg-white px-2.5 py-1 rounded-lg border border-teal-300 font-bold hover:bg-teal-50 cursor-pointer shadow-2xs"
                              >
                                Auto-fill Selected Total (₹{
                                  payableJobs
                                    .filter(j => selectedMultiJobIds.includes(j.id))
                                    .reduce((sum, j) => sum + Math.max(0, getEffectiveBillAmount(j) - (Number(j.advanceAmount) || 0)), 0)
                                })
              ...
              <div className="space-y-1">
                <label className="block font-bold text-slate-700 uppercase text-[10px]">Remarks / Notes</label>
                <input
                  type="text"
                  placeholder="e.g., Full clearance payment / Lump-sum advance"
                  value={remarks}
              ...
              <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-5 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold transition cursor-pointer text-xs shadow-xs"
                >
                  Credit Payment & Settle
                </button>
              </div>
            </form>
          </div>
