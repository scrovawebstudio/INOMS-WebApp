...
import { X, Printer, Receipt, QrCode, FileText, CheckCircle2, Cloud, ExternalLink, Copy, Check } from 'lucide-react';
import { Invoice, CompanyConfig } from '../types';
import { generateInvoicePdfBlob } from '../lib/invoicePdfService';
...
export default function InvoiceViewModal({ invoice, companyConfig, onClose, activeTenant }: InvoiceViewModalProps) {
  const [isUploadingToDrive, setIsUploadingToDrive] = useState(false);
  const [driveResult, setDriveResult] = useState<{ webViewLink?: string; success: boolean } | null>(null);
  const [copiedLink, setCopiedLink] = useState(false);
  const [uploadError, setUploadError] = useState<string | null>(null);

  if (!invoice) return null;

  const canUseDrive = hasCapability(activeTenant, 'google_drive_backup') || isTenantProPlan(activeTenant);

  const handleSaveToDrive = async () => {
    if (!canUseDrive) {
      setUploadError('Google Drive Cloud Archiving is exclusive to INOMS Pro subscribers. Please upgrade your plan.');
      return;
    }
    setIsUploadingToDrive(true);
    setUploadError(null);
    try {
      const blob = await generateInvoicePdfBlob(invoice, companyConfig);
      const safeCompanyName = (companyConfig.name || 'Enterprise').replace(/[^a-zA-Z0-9]/g, '_');
      const fileName = `Invoice_${invoice.id}_${safeCompanyName}.pdf`;
      const res = await uploadDocumentToDrive(
        fileName,
        blob,
        `Tax Invoice #${invoice.id} for ${invoice.clientName || 'Client'} generated on ${new Date().toLocaleString()}`
      );
      setDriveResult(res);
    } catch (e: any) {
      console.error('Failed to save to drive:', e);
      setUploadError(e?.message || 'Could not upload to Google Drive.');
    } finally {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl border border-slate-100 shadow-2xl max-w-4xl w-full my-8 overflow-hidden animate-slide-up cursor-default flex flex-col max-h-[92vh]"
        ...
        <div className="bg-slate-900 p-4 text-white flex justify-between items-center shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {driveResult?.webViewLink ? (
              <div className="flex items-center gap-1.5 bg-sky-950/80 border border-sky-600/50 rounded-lg px-2.5 py-1">
                ...
                <a
                  ...
                  className="p-1 hover:bg-sky-800 text-sky-200 rounded transition cursor-pointer"
                ...
                <button
                  ...
                  className="p-1 hover:bg-sky-800 text-sky-200 rounded transition cursor-pointer"
                  title="Copy Drive Link to Share"
                >
              ...
              <button
                ...
                className="flex items-center gap-1.5 bg-sky-600 hover:bg-sky-700 disabled:opacity-50 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition cursor-pointer shadow-xs"
                title={
                  canUseDrive
                    ? isTenantTrialActive()
                      ? "Save this PDF to Google Drive & generate share link (7-Day Trial - Pro Access)"
                      : "Save this PDF to Google Drive & generate share link"
                    : "Google Drive Cloud Archiving (INOMS Pro Feature)"
                }
              >
                <Cloud className={`w-3.5 h-3.5 ${isUploadingToDrive ? 'animate-bounce' : ''}`} />
                <span>{isUploadingToDrive ? 'Uploading...' : 'Save to Drive'}</span>
                {!canUseDrive && (
                  <span className="text-[9px] font-black uppercase px-1.5 py-0.2 rounded bg-purple-500 text-white border border-purple-400 leading-tight">
                    PRO
                  </span>
                )}
              </button>
