...
export default function Reports({
  ...
  // Engineer ratings computed dynamically from actual jobs
  const technicianMap = new Map<string, { assigned: number; completed: number; pending: number; revenue: number }>();
  filteredJobs.forEach(j => {
    const tech = j.assignedTechnician?.trim();
    if (!tech) return;
    const current = technicianMap.get(tech) || { assigned: 0, completed: 0, pending: 0, revenue: 0 };
    current.assigned += 1;
    const isDone = j.status === 'Outwarded' || j.status === 'Completed' || j.status === 'Product Out';
    if (isDone) {
      current.completed += 1;
      current.revenue += getEffectiveBillAmount(j);
    } else {
      current.pending += 1;
    }
    technicianMap.set(tech, current);
  });

  const engineerStats = Array.from(technicianMap.entries()).map(([name, stat]) => ({
    name,
    assigned: stat.assigned,
    completed: stat.completed,
    pending: stat.pending,
    rate: stat.assigned > 0 ? (stat.completed / stat.assigned) * 100 : 0,
              ...
              <div className="p-4 space-y-3 min-h-48 text-xs">
                {daybookPayments.length > 0 ? (
                  daybookPayments.map((p, idx) => (
                    <div key={`p-${p.id}-${idx}`} className="flex justify-between items-center border-b border-slate-100 pb-2.5 group hover:bg-slate-50/50 px-1.5 -mx-1.5 rounded-lg transition">
                      <div className="flex-1 mr-2">
                        <p className="font-bold text-slate-700">{p.clientName}</p>
                        <p className="text-[10px] text-slate-400 font-mono">
                          {p.date ? p.date.substring(0, 10) : ''} • Mode: {p.mode} {p.refNo ? `(${p.refNo})` : ''}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {onDeletePayment && (
                          <button
                            type="button"
                            onClick={() => {
                              if (confirm(`Are you sure you want to delete payment receipt #${p.id} of ₹${p.amount.toFixed(2)} for ${p.clientName}? This will restore the client's balance.`)) {
              ...
              <div className="p-4 space-y-3 min-h-48 text-xs">
                {daybookExpenses.length > 0 ? (
                  daybookExpenses.map((e, idx) => (
                    <div key={`e-${e.id}-${idx}`} className="flex justify-between items-center border-b border-slate-100 pb-2.5 group hover:bg-slate-50/50 px-1.5 -mx-1.5 rounded-lg transition">
                      <div className="flex-1 mr-2">
                        <p className="font-bold text-slate-700">{e.category}</p>
                        <p className="text-[10px] text-slate-400 italic">
                          {e.date ? e.date.substring(0, 10) : ''} • {e.remarks || 'No notes'}
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {onDeleteExpense && (
                          <button
                            type="button"
                            onClick={() => {
                              if (confirm(`Are you sure you want to delete expense record of ₹${e.amount.toFixed(2)} (${e.category})?`)) {
                ...
                {engineerStats.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={7} className="py-12 text-center text-slate-400 italic">
                      No technician assignments or repair jobs found for this period.
                    </td>
                  </tr>
                )}
              </tbody>
