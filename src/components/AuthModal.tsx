...
export interface TenantFeatures {
  ...
  allowAndroidApp?: boolean;
  allowTechnicianLANSync?: boolean;
...
export function getTenantFeatures(tenantOrFeatures?: TenantOrg | TenantFeatures | null): Required<TenantFeatures> {
  const isTenant = Boolean(tenantOrFeatures && typeof tenantOrFeatures === 'object' && 'features' in tenantOrFeatures);
  const tenant = isTenant ? (tenantOrFeatures as TenantOrg) : null;
  const f = ((tenant ? tenant.features : tenantOrFeatures) || {}) as TenantFeatures;
  const isTrial = Boolean(tenant?.isTrial || tenant?.subscriptionPlan === 'trial');
  const isMasterAdmin = tenant?.id === 'org-admin' || tenant?.code === 'ADMIN-00';

  // Master Admin has everything enabled by default
  if (isMasterAdmin) {
    return {
      allowLiveQueue: true,
      allowHomeServerSync: true,
      allowAndroidApp: true,
      allowTechnicianLANSync: true,
      ...
      allowedModules: [
        'dashboard', 'live_queue', 'inwards', 'outwards', 'billing', 'payments',
        'clients', 'inventory', 'purchases', 'suppliers', 'service_partners',
  ...
  const isPro = Boolean(
    ...
    (tenant as any)?.planTier === 'pro' ||
    (tenant as any)?.tier === 'pro' ||
    (tenant?.subscriptionPlan as string)?.toLowerCase().includes('pro')
  );
  const isTrialActive = isTenantTrialActive(tenant) || isTrial;

  // If explicitly specified in features, strictly respect the boolean value.
  // Home Server Sync, Android App and Technician LAN are strictly Pro features.
  const allowLiveQueue = typeof f.allowLiveQueue === 'boolean' ? f.allowLiveQueue : isTrial;
  const allowHomeServerSync = typeof f.allowHomeServerSync === 'boolean' ? f.allowHomeServerSync : isPro;
  const allowAndroidApp = typeof f.allowAndroidApp === 'boolean' ? f.allowAndroidApp : (isPro || isTrialActive);
  const allowTechnicianLANSync = typeof f.allowTechnicianLANSync === 'boolean' ? f.allowTechnicianLANSync : (isPro || isTrialActive);
  const allowBarcodeQrTags = typeof f.allowBarcodeQrTags === 'boolean' ? f.allowBarcodeQrTags : isTrial;
  const allowTechnicianAccounts = typeof f.allowTechnicianAccounts === 'boolean' ? f.allowTechnicianAccounts : (isPro || isTrialActive);
  const allowOutwardTaxInvoiceButton = typeof f.allowOutwardTaxInvoiceButton === 'boolean' ? f.allowOutwardTaxInvoiceButton : isTrial;
  const allowWhatsAppMessaging = typeof f.allowWhatsAppMessaging === 'boolean' ? f.allowWhatsAppMessaging : true;
  const allowGoogleDriveSync = typeof f.allowGoogleDriveSync === 'boolean' ? f.allowGoogleDriveSync : (isPro || isTrialActive);

  let allowedModules: string[];
  if (Array.isArray(f.allowedModules) && f.allowedModules.length > 0) {
    allowedModules = [...f.allowedModules];
  } else if (isTrial) {
    allowedModules = [
      'dashboard', 'live_queue', 'inwards', 'outwards', 'billing', 'payments',
      'clients', 'inventory', 'purchases', 'suppliers', 'service_partners',
    ...
    allowedModules = [
      'dashboard', 'inwards', 'outwards', 'billing', 'payments',
      'clients', 'inventory', 'expenses', 'reports', 'settings'
    ];
    if (allowLiveQueue && !allowedModules.includes('live_queue')) {
      allowedModules.push('live_queue');
    }
  }
  ...
  return {
    ...
    allowHomeServerSync,
    allowAndroidApp,
    allowTechnicianLANSync,
    allowBarcodeQrTags,
    allowWhatsAppMessaging: true, // Always available
    allowTechnicianAccounts,
    allowOutwardTaxInvoiceButton,
    allowGoogleDriveSync,
    allowedModules,
  };
}
...
export function extractSubscriberDigits(input: string): string {
  if (!input) return '';
...
export function findMatchingTenant(input: string, list: TenantOrg[]): TenantOrg | null {
  ...
  const sanitizedList = list.filter(t => t && t.id && t.id !== 'global_system_branding' && t.code !== 'GLOBAL_SYS');
  if (sanitizedList.length === 0) return null;
  const raw = input.trim();
  if (raw === '+91' || raw === '+91 ' || raw === '+1' || raw === '+44' || raw === '+' || raw === '91') return null;

  const rawLower = raw.toLowerCase();
  const rawUpper = raw.toUpperCase();
  const subDigits = extractSubscriberDigits(input);
  const isAlpha = /[a-zA-Z]/.test(raw) && raw.length >= 3;
  ...
  // 1. Full 10-digit mobile match
  if (subDigits && subDigits.length >= 10) {
    for (const t of sanitizedList) {
      if (!t) continue;
      const isMasterAdminOrg = t.id === 'org-admin' || t.code === 'ADMIN-00' || (t.ownerMobile && extractSubscriberDigits(t.ownerMobile).includes('8149862034'));
      if (isMasterAdminOrg && !isMasterAdminQuery) continue;

      const tSubDigits = extractSubscriberDigits(t.ownerMobile || '');
      if (tSubDigits && (tSubDigits === subDigits || tSubDigits.endsWith(subDigits) || subDigits.endsWith(tSubDigits))) {
        return t;
      }
    }
  }

  // 2. Prefix phone match (starts fetching & matching as user types 5+ digits)
  if (subDigits && subDigits.length >= 5) {
    for (const t of sanitizedList) {
      if (!t) continue;
      const isMasterAdminOrg = t.id === 'org-admin' || t.code === 'ADMIN-00';
      if (isMasterAdminOrg && !isMasterAdminQuery) continue;

      const tSubDigits = extractSubscriberDigits(t.ownerMobile || '');
      if (tSubDigits && (tSubDigits.startsWith(subDigits) || tSubDigits === subDigits)) {
        return t;
      }
    }
  }

  // 3. Exact code or exact ID or exact name match
  if (isAlpha && rawUpper) {
    for (const t of sanitizedList) {
      if (!t) continue;
      const isMasterAdminOrg = t.id === 'org-admin' || t.code === 'ADMIN-00';
      ...
      if ((t.code && t.code.toUpperCase() === rawUpper) || (t.id && t.id.toUpperCase() === rawUpper)) {
        return t;
      }
      if ((t.name && t.name.toLowerCase() === rawLower) || (t.ownerName && t.ownerName.toLowerCase() === rawLower)) {
        return t;
      }
    }
  }

  // 3. Partial substring search on name only if explicit alphanumeric name >= 4 characters
  if (!isMasterAdminQuery && isAlpha && rawLower.length >= 4) {
    for (const t of sanitizedList) {
      if (t.id === 'org-admin' || t.code === 'ADMIN-00') continue;
      if (t.name && t.name.toLowerCase().includes(rawLower)) {
        return t;
      }
    }
  }

  // 4. If query specifically matches master admin, return master admin org
  if (isMasterAdminQuery) {
    return sanitizedList.find(t => t.id === 'org-admin' || t.code === 'ADMIN-00' || (t.ownerMobile && extractSubscriberDigits(t.ownerMobile).includes('8149862034'))) || null;
  }
...
export default function AuthModal({
  ...
  const [expiredOrgTarget, setExpiredOrgTarget] = useState<TenantOrg | null>(null);

  // Debounce refs for API lookup
  const lookupDebounceRef = useRef<NodeJS.Timeout | null>(null);
  const staffLookupDebounceRef = useRef<NodeJS.Timeout | null>(null);
  const lookupRequestIdRef = useRef<number>(0);
  const staffLookupRequestIdRef = useRef<number>(0);
  ...
  useEffect(() => {
    return () => {
      ...
      if (staffLookupDebounceRef.current) clearTimeout(staffLookupDebounceRef.current);
    };
  }, []);
  ...
  const handleStaffOwnerMobileChange = (val: string) => {
    ...
    setStaffError('');
    const raw = val.trim();
    const isJustPrefix = raw === '+91' || raw === '+91 ' || raw === '+1' || raw === '+44' || raw === '+' || raw === '91' || raw === '';
    const subDigits = extractSubscriberDigits(val);
    const hasEnoughDigits = subDigits.length >= 5;
    const isAlpha = /[a-zA-Z]/.test(val) && raw.length >= 3;

    if (isJustPrefix || (!hasEnoughDigits && !isAlpha)) {
      if (staffLookupDebounceRef.current) clearTimeout(staffLookupDebounceRef.current);
    ...
    if (match) {
      setStaffDetectedOrg(match);
      setStaffTenantId(match.id);
    }

    // 2. Debounced API fetch (250ms) to avoid spamming the backend
    if (staffLookupDebounceRef.current) clearTimeout(staffLookupDebounceRef.current);
    const reqId = ++staffLookupRequestIdRef.current;

    staffLookupDebounceRef.current = setTimeout(async () => {
      const query = hasEnoughDigits ? subDigits : raw;
      try {
        const res = await lookupOrgByMobileViaApi(query);
        if (staffLookupRequestIdRef.current === reqId) {
        ...
        if (staffLookupRequestIdRef.current === reqId && !match) {
          setStaffDetectedOrg(null);
        }
      }
    }, 250);
    ...
    if (isOpen) {
      ...
      try {
        ...
        if (savedMobile) {
          ...
          const rawSaved = (parsed?.mobileInput || '').toString().trim();
          const subDigits = extractSubscriberDigits(rawSaved);
          const isJustPrefix = rawSaved === '+91' || rawSaved === '+91 ' || rawSaved === '+1' || rawSaved === '+44' || rawSaved === '+' || rawSaved === '91' || rawSaved === '';
          if (rawSaved && !isJustPrefix && (subDigits.length >= 5 || (/[a-zA-Z]/.test(rawSaved) && rawSaved.length >= 3))) {
            ...
            lookupOrgByMobileViaApi(subDigits.length >= 5 ? subDigits : rawSaved).then(res => {
              if (res.success && res.org) {
      ...
      try {
        ...
        if (savedStaff) {
          ...
          if (parsed) {
            const rawStaffMobile = (parsed.ownerMobile || '').toString().trim();
            const subDigits = extractSubscriberDigits(rawStaffMobile);
            const isJustPrefix = rawStaffMobile === '+91' || rawStaffMobile === '+91 ' || rawStaffMobile === '+1' || rawStaffMobile === '+44' || rawStaffMobile === '+' || rawStaffMobile === '91' || rawStaffMobile === '';
            if (rawStaffMobile && !isJustPrefix && (subDigits.length >= 5 || (/[a-zA-Z]/.test(rawStaffMobile) && rawStaffMobile.length >= 3))) {
              ...
              lookupOrgByMobileViaApi(subDigits.length >= 5 ? subDigits : rawStaffMobile).then(res => {
                if (res.success && res.org) {
                  setStaffDetectedOrg(res.org);
                  setStaffTenantId(res.org.id);
                }
  ...
  // Dynamic search with debounce as user types mobile number or organization code (triggers on 5 digits)
  const handleMobileInputChange = (value: string) => {
    ...
    const isJustPrefix = raw === '+91' || raw === '+91 ' || raw === '+1' || raw === '+44' || raw === '+' || raw === '91' || raw === '';
    const subDigits = extractSubscriberDigits(value);
    const hasEnoughDigits = subDigits.length >= 5;
    const isAlpha = /[a-zA-Z]/.test(value) && raw.length >= 3;
    ...
    // 1. Instant local matching if workspace is already cached or in memory
    const localMatch = findMatchingTenant(value, tenants);
    if (localMatch) {
      setDetectedTenant(localMatch);
    }

    // 2. Debounce server API lookup (250ms delay) to prevent repeated network spam
    if (lookupDebounceRef.current) clearTimeout(lookupDebounceRef.current);
    const reqId = ++lookupRequestIdRef.current;
    setIsSearchingWorkspace(!localMatch);

    lookupDebounceRef.current = setTimeout(async () => {
      const query = hasEnoughDigits ? subDigits : raw;
      try {
        const res = await lookupOrgByMobileViaApi(query);
        if (lookupRequestIdRef.current === reqId) {
          setIsSearchingWorkspace(false);
          if (res.success && res.org) {
            setDetectedTenant(res.org);
          } else if (!localMatch) {
  ...
  const handleRequestApproval = async (e: React.FormEvent) => {
    ...
    const raw = mobileInput.trim();
    const isJustPrefix = raw === '+91' || raw === '+91 ' || raw === '+1' || raw === '+44' || raw === '+' || raw === '91' || raw === '';
    const subDigits = extractSubscriberDigits(mobileInput);
    const hasEnoughDigits = subDigits.length >= 5;
    const isAlpha = /[a-zA-Z]/.test(mobileInput) && raw.length >= 3;

    if (isJustPrefix || (!hasEnoughDigits && !isAlpha)) {
      setMobileError('Please enter at least 5 digits of your registered mobile number or workspace code.');
      return;
    }
    ...
    try {
      const lookupRes = await lookupOrgByMobileViaApi(hasEnoughDigits ? subDigits : raw);
      let match = (lookupRes.success && lookupRes.org) ? lookupRes.org : localMatch;

      if (match) {
        // Check if organization is expired or deactivated
        const isMaster = match.id === 'org-admin' || match.code?.toUpperCase() === 'ADMIN-00' || match.ownerMobile?.includes('8149862034');
        let isExpired = false;
        if (!isMaster && match.subscriptionEndDate) {
          const end = new Date(match.subscriptionEndDate);
        ...
        if (match.status === 'deactivated' || isExpired) {
          setDetectedTenant(match);
          setExpiredOrgTarget(match);
          setMobileSubmitted(false);
          setMobileError(`🔒 SUBSCRIPTION EXPIRED / DEACTIVATED: Organization "${match.name}" access is expired or suspended.`);
  ...
  const handleTotpVerificationSubmit = async (e: React.FormEvent) => {
    ...
    const cleanMobile = (detectedTenant.ownerMobile || '').replace(/\D/g, '');
    const isMasterAdmin = detectedTenant.id === 'org-admin' || cleanMobile === '8149862034' || detectedTenant.code === 'ADMIN-00';
    ...
    if (isValid) {
      if (rememberMeMobile && !isMasterAdmin) {
        localStorage.setItem('remembered_login_mobile', JSON.stringify({ mobileInput }));
      } else {
        localStorage.removeItem('remembered_login_mobile');
      }
      ...
      setTimeout(() => {
        onAuthenticated(detectedTenant, isMasterAdmin ? 'Admin' : 'Org Admin');
      }, 500);
    } else {
      setIsVerifyingTotp(false);
  ...
  const handleStaffSubmit = async (e: React.FormEvent) => {
    ...
    // Fallback match check only if server was offline/unreachable
    if (!apiResult.message) {
      ...
      if (match) {
        ...
        const userPassword = (match.password || match.pin || '').trim();
        const isCorrectPassword = Boolean(userPassword && cleanPass === userPassword);
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    if (cleanCode.length !== 6) {
      setPinError('Please enter a valid 6-digit passcode or Master PIN.');
      return;
    }
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-slate-900/80 backdrop-blur-md cursor-pointer overflow-y-auto"
      onClick={(e) => {
    ...
    >
      <div 
        className="bg-white rounded-none sm:rounded-3xl shadow-2xl border-0 sm:border sm:border-slate-100 w-full min-h-screen sm:min-h-0 sm:max-w-2xl overflow-y-auto sm:overflow-hidden animate-in fade-in zoom-in duration-200 cursor-default flex flex-col justify-between"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Header */}
        <div className="bg-gradient-to-r from-slate-900 via-teal-950 to-slate-900 p-4 sm:p-6 text-white flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            {onClose && (
              <button
                type="button"
                onClick={onClose}
                className="p-2 text-slate-400 hover:text-white hover:bg-slate-800/80 rounded-xl transition cursor-pointer min-h-[44px] min-w-[44px] flex items-center justify-center"
                title="Close Window"
              >
                <X className="w-5 h-5" />
              </button>
            ...
            <div className="space-y-4">
              ...
              <form onSubmit={handleRequestApproval} className="space-y-3">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
                    ...
                    {detectedTenant ? (
                      ...
                      <span className="text-[10px] text-teal-600 font-medium flex items-center gap-1 animate-pulse">
                        <Loader2 className="w-3 h-3 animate-spin" /> Detecting workspace...
                      </span>
                    ) : (
                ...
                {mobileError && (
                  <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-2xl space-y-2">
                    ...
                    {detectedTenant && (detectedTenant.status === 'deactivated' || detectedTenant.subscriptionEndDate) && (
                      <button
                        type="button"
                        onClick={() => {
                          setExpiredOrgTarget(detectedTenant);
                          setShowExpiredModal(true);
                        }}
                        className="w-full bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white font-extrabold text-xs py-2 px-3 rounded-xl transition cursor-pointer flex items-center justify-center gap-2 shadow-sm"
                      >
                        <Sparkles className="w-3.5 h-3.5 text-amber-300" />
                        <span>Renew Subscription Directly & Reactivate Workspace</span>
                      </button>
                    )}
                  </div>
                )}
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 p-3.5 rounded-2xl border border-emerald-200/80">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-xs font-black text-slate-900">New to INOMS?</p>
      ...
      {/* Subscription Expired / Renewal Direct Modal */}
      {showExpiredModal && expiredOrgTarget && (
        <SubscriptionExpiredModal
          tenant={expiredOrgTarget}
          onRenewRequested={(req: SubscriptionRenewalRequest) => {
            try {
              const currentRequests = JSON.parse(localStorage.getItem('inoms_subscription_renewal_requests_v1') || '[]');
              const updated = [req, ...currentRequests.filter((r: any) => r.id !== req.id)];
              localStorage.setItem('inoms_subscription_renewal_requests_v1', JSON.stringify(updated));
