...
export async function bootstrapTenantFromHomeServer(tenantId: string): Promise<{
  ...
  try {
    const res = await fetch(resolveApiUrl(`/api/sync/bootstrap?tenantId=${encodeURIComponent(tenantId)}`), { headers });
    if (!res.ok) {
    ...
    let collections: Record<string, any[]> = data.collections || {};
    const totalServerRecords: number = Object.values(collections).reduce((acc: number, arr: any) => acc + (Array.isArray(arr) ? arr.length : 0), 0);
    const serverRev: number = typeof data.serverRevision === 'number' ? data.serverRevision : Number(data.serverRevision || 0);

    const isServerAuthoritative = Boolean(totalServerRecords > 0 || serverRev > 1);

    // Only if the server has 0 records AND serverRevision <= 1 (brand-new/unseeded server),
    // seed the server with existing single-device local storage data.
    if (!isServerAuthoritative) {
      ...
      if (localHasData) {
        ...
        if (token) {
          await fetch(resolveApiUrl('/api/sync/save-all'), {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'x-tenant-id': tenantId,
...
export async function pullDeltaFromHomeServer(tenantId: string): Promise<number> {
  ...
  try {
    ...
    const res = await fetch(resolveApiUrl(`/api/sync/pull?sinceRevision=${sinceRevision}&tenantId=${encodeURIComponent(tenantId)}`), { headers });
    if (!res.ok) {
    ...
    // Notify affected UI components and keep localStorage in sync
    for (const ent of affectedEntities) {
      const fresh = await getLocalCollection(tenantId, ent);
      try {
        setAppStorageItem(`${ent}_${tenantId}`, JSON.stringify(fresh));
      } catch (_) {}
...
export async function pushPendingOperations(tenantId: string): Promise<{
  ...
  try {
    ...
    const res = await fetch(resolveApiUrl('/api/sync/push'), {
      method: 'POST',
      headers,
