...
export function generateJobWhatsAppMessage(job: RepairJob, companyConfig: CompanyConfig): string {
  ...
  if (companyConfig.googleReviewUrl && companyConfig.googleReviewUrl.trim()) {
    socialLinks.push(`⭐ *Review us on Google:* ${companyConfig.googleReviewUrl.trim()}`);
  } else if (companyConfig.customReviewUrl && companyConfig.customReviewUrl.trim()) {
    socialLinks.push(`⭐ *Rate our Service:* ${companyConfig.customReviewUrl.trim()}`);
  }
  if (companyConfig.whatsappChannelUrl && companyConfig.whatsappChannelUrl.trim()) {
    socialLinks.push(`📢 *Follow on WhatsApp Channel:* ${companyConfig.whatsappChannelUrl.trim()}`);
  }
  if (companyConfig.instagramUrl && companyConfig.instagramUrl.trim()) {
    socialLinks.push(`📸 *Follow on Instagram:* ${companyConfig.instagramUrl.trim()}`);
  }
  if (companyConfig.facebookUrl && companyConfig.facebookUrl.trim()) {
    socialLinks.push(`👍 *Connect on Facebook:* ${companyConfig.facebookUrl.trim()}`);
  }
  if (companyConfig.youtubeUrl && companyConfig.youtubeUrl.trim()) {
    socialLinks.push(`▶️ *YouTube:* ${companyConfig.youtubeUrl.trim()}`);
  }
