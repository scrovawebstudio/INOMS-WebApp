...
export async function initDatabase(): Promise<Database> {
  ...
  // Safely wrap db.run and db.prepare so that undefined parameters in arrays are converted to null,
  // preventing sql.js "Wrong API use : tried to bind a value of an unknown type (undefined)" exceptions
  const origRun = db.run.bind(db);
  db.run = (sql: string, params?: any[]) => {
    const safeParams = Array.isArray(params) ? params.map(p => p === undefined ? null : p) : params;
    return origRun(sql, safeParams);
  };
  const origPrepare = db.prepare.bind(db);
  db.prepare = (sql: string, params?: any[]) => {
    const safeParams = Array.isArray(params) ? params.map(p => p === undefined ? null : p) : params;
    const stmt = origPrepare(sql, safeParams);
    const origBind = stmt.bind.bind(stmt);
    stmt.bind = (values?: any[]) => {
      const safeVals = Array.isArray(values) ? values.map(v => v === undefined ? null : v) : values;
      return origBind(safeVals);
    };
    return stmt;
  };
  ...
  db.run(`
    ...
    -- Deleted Organizations Tombstone
    ...
    -- Service Partners (External Repair Vendors / Motherboard Specialists)
    ...
    -- Purchases (Inward Material / Invoices from Suppliers)
    ...
    -- Supplier Payments (Outward Payments to Suppliers)
    ...
    CREATE TABLE IF NOT EXISTS service_partner_payments (
      id TEXT PRIMARY KEY,
      organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
      tenant_id TEXT,
    ...
    -- Inventory Transactions (Stock In / Stock Out / Scrap / Adjustment / Job Used)
    CREATE TABLE IF NOT EXISTS inventory_transactions (
      id TEXT PRIMARY KEY,
      organization_id TEXT NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
      tenant_id TEXT,
...
async function migrateSqliteSchema(database: Database): Promise<void> {
  try {
    ...
    // Universal column guarantee across all entity tables
    const entityTables = [
      'organizations', 'subscriptions', 'organization_features', 'organization_users',
      'users', 'tenant_configs', 'clients', 'jobs', 'job_status_history', 'outward',
      'invoices', 'invoice_items', 'payments', 'products', 'inventory_transactions',
      'expenses', 'ledger_entries', 'ledger', 'categories', 'racks', 'equipments',
      'problems', 'documents', 'audit_logs', 'suppliers', 'service_partners',
      'purchase_orders', 'purchases', 'purchase_returns', 'supplier_payments',
      'service_partner_payments', 'inventory_serials'
    ];
    ...
    ensureColumns('organization_users', [
      ...
      { name: 'status', type: 'TEXT DEFAULT "Active"' },
      { name: 'is_deactivated', type: 'INTEGER DEFAULT 0' },
    ...
    // 13. Ensure columns for suppliers & service_partners & inventory extensions
    ensureColumns('suppliers', [
      ...
      { name: 'contact_person', type: 'TEXT' },
      { name: 'mobile', type: 'TEXT' },
      { name: 'alternate_number', type: 'TEXT' },
      { name: 'email', type: 'TEXT' },
      { name: 'address', type: 'TEXT' },
      { name: 'city', type: 'TEXT' },
      { name: 'state', type: 'TEXT' },
    ...
    ensureColumns('service_partners', [
      { name: 'organization_id', type: 'TEXT' },
      { name: 'tenant_id', type: 'TEXT' },
      { name: 'company_name', type: 'TEXT' },
      { name: 'contact_person', type: 'TEXT' },
    ...
    ensureColumns('purchases', [
      { name: 'organization_id', type: 'TEXT' },
      { name: 'tenant_id', type: 'TEXT' },
      { name: 'supplier_id', type: 'TEXT' },
      { name: 'supplier_name', type: 'TEXT' },
      { name: 'supplier_invoice_no', type: 'TEXT' },
      ...
      { name: 'tax_amount', type: 'REAL DEFAULT 0' },
      { name: 'shipping_charges', type: 'REAL DEFAULT 0' },
      { name: 'round_off', type: 'REAL DEFAULT 0' },
      { name: 'grand_total', type: 'REAL DEFAULT 0' },
    ...
    ensureColumns('purchase_returns', [
      ...
      { name: 'tax_amount', type: 'REAL DEFAULT 0' },
      { name: 'grand_total', type: 'REAL DEFAULT 0' },
      { name: 'refund_status', type: 'TEXT DEFAULT "Pending"' },
      { name: 'notes', type: 'TEXT' },
      { name: 'data_json', type: 'TEXT' }
    ]);
    ...
    ensureColumns('supplier_payments', [
      ...
      { name: 'date', type: 'TEXT' },
      { name: 'amount', type: 'REAL DEFAULT 0' },
      { name: 'payment_mode', type: 'TEXT DEFAULT "Bank Transfer"' },
      { name: 'reference_number', type: 'TEXT' },
      { name: 'notes', type: 'TEXT' },
      { name: 'data_json', type: 'TEXT' }
    ]);

    ensureColumns('service_partner_payments', [
      { name: 'organization_id', type: 'TEXT' },
      { name: 'tenant_id', type: 'TEXT' },
      { name: 'service_partner_id', type: 'TEXT' },
      { name: 'service_partner_name', type: 'TEXT' },
    ...
    ensureColumns('inventory_serials', [
      { name: 'organization_id', type: 'TEXT' },
      { name: 'tenant_id', type: 'TEXT' },
      ...
      { name: 'purchase_id', type: 'TEXT' },
      { name: 'supplier_id', type: 'TEXT' },
      { name: 'purchase_rate', type: 'REAL DEFAULT 0' },
      { name: 'status', type: 'TEXT DEFAULT "In Stock"' },
      { name: 'assigned_job_id', type: 'TEXT' },
      { name: 'assigned_client_id', type: 'TEXT' },
      { name: 'sold_invoice_id', type: 'TEXT' },
      { name: 'warranty_months', type: 'INTEGER DEFAULT 0' },
      { name: 'warranty_expiry', type: 'TEXT' },
      { name: 'notes', type: 'TEXT' },
      { name: 'data_json', type: 'TEXT' }
    ]);

    ensureColumns('inventory_transactions', [
      { name: 'organization_id', type: 'TEXT' },
      { name: 'tenant_id', type: 'TEXT' },
      { name: 'product_id', type: 'TEXT' },
      { name: 'product_name', type: 'TEXT' },
    ...
    const syncOrgIdTables = [
      'clients', 'jobs', 'invoices', 'payments', 'products', 'expenses', 'ledger',
      'categories', 'racks', 'equipments', 'problems', 'audit_logs',
      'suppliers', 'service_partners', 'purchase_orders', 'purchases', 'purchase_returns',
      'supplier_payments', 'service_partner_payments', 'inventory_serials', 'inventory_transactions'
    ];
      ...
      try {
        ...
        if (path.basename(filePath) === 'organizations.json' && Array.isArray(parsed)) {
          for (const org of parsed) {
            ...
            if (isOrganizationDeleted(db, String(org.id)) || org.id === 'global_system_branding') continue;
            ...
            const isActiveNum = orgStatus === 'active' ? 1 : 0;
            const subStatus = orgStatus === 'active' ? 'active' : 'suspended';

            db.run(
              `INSERT OR REPLACE INTO organizations (
                id, organization_code, organization_name, name, code, owner_mobile, owner_name, status, is_active, subscription_status, secret_key, pin, pin_hash, pin_salt,
              ...
              [
                ...
                org.ownerMobile || null, org.ownerName || null, orgStatus, isActiveNum, subStatus,
        ...
        if (targetTenantId && targetTenantId !== 'org-admin') {
          if (isOrganizationDeleted(db, targetTenantId) || targetTenantId === 'global_system_branding') continue;
          ...
          if (!orgExists) {
            db.run(
              `INSERT OR REPLACE INTO organizations (
                id, organization_code, organization_name, name, code, owner_mobile, owner_name, status, is_active, subscription_status,
          ...
          if (Array.isArray(items)) {
            for (const item of items) {
              ...
              if (tbl === 'clients') {
                ...
                db.run(
                  `INSERT OR REPLACE INTO users (id, organization_id, tenant_id, name, mobile, role, username, data_json, created_at, updated_at, version)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [item.id, orgId, orgId, item.name || 'User', item.mobile || '', item.role || 'technician', item.username || item.name || '', dataJson, now, now]
                );
                counts.users++;
              } else if (tbl === 'categories') {
                db.run(
                  `INSERT OR REPLACE INTO categories (id, organization_id, tenant_id, name, description, data_json, created_at, updated_at, version)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [item.id, orgId, orgId, item.name || 'Category', item.description || null, dataJson, now, now]
                );
                counts.categories++;
              } else if (tbl === 'racks') {
                db.run(
                  `INSERT OR REPLACE INTO racks (id, organization_id, tenant_id, name, rack_code, capacity, location, description, status, data_json, created_at, updated_at, version)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [item.id, orgId, orgId, item.name || 'Rack', item.rackCode || item.code || null, item.capacity || null, item.location || null, item.description || null, item.status || 'active', dataJson, now, now]
                );
                counts.racks++;
              } else if (tbl === 'equipments') {
                db.run(
                  `INSERT OR REPLACE INTO equipments (id, organization_id, tenant_id, name, brand, model, data_json, created_at, updated_at, version)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [item.id, orgId, orgId, item.name || 'Equipment', item.brand || null, item.model || null, dataJson, now, now]
                );
                counts.equipments++;
              } else if (tbl === 'problems') {
                db.run(
                  `INSERT OR REPLACE INTO problems (id, organization_id, tenant_id, name, title, description, common_solution, standard_cost, data_json, created_at, updated_at, version)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [item.id, orgId, orgId, item.name || item.title || 'Problem', item.title || item.name || null, item.description || null, item.commonSolution || null, item.standardCost || 0, dataJson, now, now]
                );
                counts.problems++;
              } else if (tbl === 'suppliers') {
                db.run(
                  ...
                  [
                    item.id, orgId, orgId, item.name || 'Supplier', item.companyName || null, item.contactPerson || null,
                    item.mobile || '', item.alternateNumber || null, item.email || null, item.address || null,
                    item.city || null, item.state || null, item.pincode || null, item.gstin || null, item.pan || null,
                    item.openingBalance || 0, item.currentBalance || 0, item.notes || null, item.status || 'Active',
                    dataJson, now, now
                  ]
                );
                counts.suppliers++;
              } else if (tbl === 'service_partners') {
                db.run(
                  `INSERT OR REPLACE INTO service_partners (
                    id, organization_id, tenant_id, name, company_name, contact_person, mobile, alternate_number,
                    email, address, city, state, gstin, notes, status, data_json, created_at, updated_at, version
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [
                    item.id, orgId, orgId, item.name || 'Service Partner', item.companyName || null, item.contactPerson || null,
                    item.mobile || '', item.alternateNumber || null, item.email || null, item.address || null,
                    item.city || null, item.state || null, item.gstin || null, item.notes || null, item.status || 'Active',
                    dataJson, now, now
                  ]
                );
                counts.servicePartners++;
              } else if (tbl === 'purchase_orders') {
                db.run(
                  `INSERT OR REPLACE INTO purchase_orders (
                    id, organization_id, tenant_id, supplier_id, supplier_name, date, expected_delivery_date,
                    status, notes, subtotal, tax_amount, grand_total, data_json, created_at, updated_at, version
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [
                    item.id, orgId, orgId, item.supplierId || '', item.supplierName || '', item.date || now,
                    item.expectedDeliveryDate || null, item.status || 'Draft', item.notes || null,
                    item.subtotal || 0, item.taxAmount || 0, item.grandTotal || item.total || 0,
                    dataJson, now, now
                  ]
                );
                counts.purchaseOrders++;
              } else if (tbl === 'purchases') {
                db.run(
                  `INSERT OR REPLACE INTO purchases (
                    ...
                    grand_total, paid_amount, balance_amount, payment_status, notes, data_json, created_at, updated_at, version
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [
                    item.id, orgId, orgId, item.supplierId || '', item.supplierName || '', item.supplierInvoiceNo || null,
                    item.purchaseOrderId || null, item.date || now, item.dueDate || null, item.paymentType || 'Credit',
                    item.subtotal || 0, item.discount || 0, item.taxAmount || 0, item.shippingCharges || 0, item.roundOff || 0,
                    item.grandTotal || item.total || 0, item.paidAmount || 0, item.balanceAmount || 0,
                    item.paymentStatus || 'Unpaid', item.notes || null, dataJson, now, now
                  ]
                );
                counts.purchases++;
              } else if (tbl === 'purchase_returns') {
                db.run(
                  `INSERT OR REPLACE INTO purchase_returns (
                    id, organization_id, tenant_id, purchase_id, supplier_id, supplier_name, date, reason,
                    subtotal, tax_amount, grand_total, refund_status, notes, data_json, created_at, updated_at, version
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [
                    item.id, orgId, orgId, item.purchaseId || null, item.supplierId || '', item.supplierName || '',
                    item.date || now, item.reason || null, item.subtotal || 0, item.taxAmount || 0,
                    item.grandTotal || item.total || 0, item.refundStatus || 'Pending', item.notes || null,
                    dataJson, now, now
                  ]
                );
                counts.purchaseReturns++;
              } else if (tbl === 'supplier_payments') {
                db.run(
                  `INSERT OR REPLACE INTO supplier_payments (
                    id, organization_id, tenant_id, supplier_id, supplier_name, purchase_id, date, amount,
                    payment_mode, reference_number, notes, data_json, created_at, updated_at, version
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [
                    item.id, orgId, orgId, item.supplierId || '', item.supplierName || '', item.purchaseId || null,
                    item.date || now, item.amount || 0, item.paymentMode || 'Bank Transfer', item.referenceNumber || null,
                    item.notes || null, dataJson, now, now
                  ]
                );
                counts.supplierPayments++;
              } else if (tbl === 'service_partner_payments') {
                db.run(
                  `INSERT OR REPLACE INTO service_partner_payments (
                    ...
                    amount, payment_mode, reference_number, notes, data_json, created_at, updated_at, version
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [
                    item.id, orgId, orgId, item.servicePartnerId || '', item.servicePartnerName || '', item.jobId || null,
                    item.date || now, item.amount || 0, item.paymentMode || 'Bank Transfer', item.referenceNumber || null,
                    item.notes || null, dataJson, now, now
                  ]
                );
                counts.servicePartnerPayments++;
              } else if (tbl === 'inventory_serials') {
                db.run(
                  `INSERT OR REPLACE INTO inventory_serials (
                    id, organization_id, tenant_id, product_id, product_name, serial_number, status,
                    purchase_id, purchase_date, warranty_expiry_date, notes, data_json, created_at, updated_at, version
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [
                    item.id, orgId, orgId, item.productId || '', item.productName || '', item.serialNumber || '',
                    item.status || 'In Stock', item.purchaseId || null, item.purchaseDate || null,
                    item.warrantyExpiryDate || null, item.notes || null, dataJson, now, now
                  ]
                );
                counts.inventorySerials++;
              } else if (tbl === 'inventory_transactions') {
                db.run(
                  `INSERT OR REPLACE INTO inventory_transactions (
                    id, organization_id, tenant_id, product_id, product_name, transaction_type, quantity,
                    unit_price, total_amount, reference_type, reference_id, reference_number, notes,
                    transaction_date, data_json, created_at, updated_at, version
                  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
                  [
                    item.id, orgId, orgId, item.productId || '', item.productName || '', item.transactionType || 'in',
                    item.quantity || 0, item.unitPrice || 0, item.totalAmount || 0, item.referenceType || null,
                    item.referenceId || null, item.referenceNumber || null, item.notes || null,
                    item.transactionDate || now, dataJson, now, now
                  ]
                );
...
// Check if organization was permanently deleted
export function isOrganizationDeleted(database: Database, orgId: string): boolean {
  if (!database || !orgId) return false;
  try {
    const stmt = database.prepare('SELECT id FROM deleted_organizations WHERE id = ?');
    stmt.bind([orgId]);
    const isDel = stmt.step();
    stmt.free();
    return Boolean(isDel);
  } catch {
    return false;
  }
}

export function unmarkOrganizationDeleted(database: Database, orgId: string): void {
  if (!database || !orgId) return;
  try {
    database.run('DELETE FROM deleted_organizations WHERE id = ?', [orgId]);
  } catch {}
}

// Ensure Master Admin and referenced organizations exist in SQLite
export function ensureOrganizationExists(database: Database, orgId: string, name?: string): void {
  if (!database || !orgId || orgId === 'global_system_branding') return;
  // Guard: NEVER recreate an organization that was deleted!
  if (isOrganizationDeleted(database, orgId)) return;
  try {
    const stmt = database.prepare('SELECT id FROM organizations WHERE id = ?');
    stmt.bind([orgId]);
    const exists = stmt.step();
    stmt.free();
    if (!exists) {
      const now = new Date().toISOString();
      const orgName = orgId === 'org-admin' ? 'Master System Admin' : (name || orgId);
      const orgCode = orgId === 'org-admin' ? 'ADMIN-00' : orgId.toUpperCase().slice(0, 10);
      ...
      database.run(
        `INSERT OR IGNORE INTO organizations (
          ...
          status, is_active, subscription_plan, subscription_status, subscription_start_date,
          trial_days, is_trial, pin_hash, pin_salt, created_at, updated_at, version
        ) VALUES (?, ?, ?, ?, ?, ?, '', 'active', 1, 'lifetime', 'active', ?, 0, 0, ?, ?, ?, ?, 1)`,
        [orgId, orgCode, orgName, orgName, orgCode, orgName, now, hash, salt, now, now]
      );
      scheduleDbSave();
    }
  } catch (err) {
    console.warn(`[ensureOrganizationExists] Error for org ${orgId}:`, err);
  }
}
...
async function migrateLegacyDataIfPresent(): Promise<void> {
  if (!db) return;
  try {
    // Unconditionally ensure Master Admin (org-admin) always exists in organizations
    ensureOrganizationExists(db, 'org-admin', 'Master System Admin');
  } catch (err) {
...
export function exportTenantToDisk(tenantId: string): void {
  ...
  try {
    ...
    const collections = [
      'clients', 'jobs', 'invoices', 'payments', 'products', 'expenses', 'ledger', 'users', 'categories', 'racks', 'equipments', 'problems',
      'suppliers', 'service_partners', 'purchases', 'purchase_orders', 'purchase_returns', 'supplier_payments', 'service_partner_payments', 'inventory_serials', 'inventory_transactions'
    ];
    ...
    // Ensure both camelCase and snake_case properties are present in exported data
    if (tenantData.service_partners && !tenantData.servicePartners) tenantData.servicePartners = tenantData.service_partners;
    if (tenantData.purchase_orders && !tenantData.purchaseOrders) tenantData.purchaseOrders = tenantData.purchase_orders;
    if (tenantData.purchase_returns && !tenantData.purchaseReturns) tenantData.purchaseReturns = tenantData.purchase_returns;
    if (tenantData.supplier_payments && !tenantData.supplierPayments) tenantData.supplierPayments = tenantData.supplier_payments;
    if (tenantData.service_partner_payments && !tenantData.servicePartnerPayments) tenantData.servicePartnerPayments = tenantData.service_partner_payments;
    if (tenantData.inventory_serials && !tenantData.inventorySerials) tenantData.inventorySerials = tenantData.inventory_serials;
    if (tenantData.inventory_transactions && !tenantData.inventoryTransactions) tenantData.inventoryTransactions = tenantData.inventory_transactions;
...
export async function deleteOrganizationFromDatabaseAndDisk(orgId: string): Promise<boolean> {
  ...
  try {
    // 0. Record in deleted_organizations tombstone so background loops or cached syncs can never resurrect it
    try {
      db.run('CREATE TABLE IF NOT EXISTS deleted_organizations (id TEXT PRIMARY KEY, deleted_at TEXT NOT NULL)');
      db.run('INSERT OR REPLACE INTO deleted_organizations (id, deleted_at) VALUES (?, ?)', [orgId, new Date().toISOString()]);
    } catch (_) {}
...
// Automatically check and deactivate organizations whose subscription or free trial has expired
export function autoDeactivateExpiredOrganizations(): { deactivatedCount: number; deactivatedIds: string[] } {
  if (!db) return { deactivatedCount: 0, deactivatedIds: [] };
  const deactivatedIds: string[] = [];
  try {
    const stmt = db.prepare(`
      SELECT id, name, code, owner_mobile, status, subscription_plan, subscription_start_date, subscription_end_date, trial_days, is_trial, created_at
      FROM organizations
      WHERE id != 'org-admin' AND status = 'active'
    `);

    const now = Date.now();
    const nowIso = new Date().toISOString();
    const orgsToDeactivate: string[] = [];

    while (stmt.step()) {
      const org = stmt.getAsObject();
      const id = String(org.id || '');
      const code = String(org.code || '');
      const mobile = String(org.owner_mobile || '').replace(/\D/g, '');
      const plan = String(org.subscription_plan || (org.is_trial ? 'trial' : 'monthly')).toLowerCase();

      // Skip master admin and lifetime subscriptions
      if (id === 'org-admin' || code === 'ADMIN-00' || mobile.includes('8149862034') || plan === 'lifetime') {
        continue;
      }

      let endDateMs: number | null = null;
      let endDateStr = org.subscription_end_date ? String(org.subscription_end_date).trim() : '';
      ...
      // If no explicit end date, calculate from subscription start or created_at
      if (!endDateMs || isNaN(endDateMs)) {
        const startStr = (org.subscription_start_date || org.created_at) ? String(org.subscription_start_date || org.created_at) : '';
        const startMs = startStr ? new Date(startStr).getTime() : NaN;
        if (!isNaN(startMs)) {
          if (org.is_trial || plan.includes('trial') || plan.includes('free')) {
            const trialDays = Number(org.trial_days) > 0 ? Number(org.trial_days) : 7;
            endDateMs = startMs + trialDays * 24 * 60 * 60 * 1000;
          } else if (plan.includes('month') || plan === '1month') {
            endDateMs = startMs + 30 * 24 * 60 * 60 * 1000;
          } else if (plan.includes('year') || plan === '1year' || plan === '12month') {
      ...
      if (endDateMs && !isNaN(endDateMs) && endDateMs < now) {
        orgsToDeactivate.push(id);
      }
    }
    stmt.free();

    if (orgsToDeactivate.length > 0) {
      for (const orgId of orgsToDeactivate) {
        ...
        deactivatedIds.push(orgId);
        console.log(`[Subscription Expiry] Automatically deactivated expired organization ID: ${orgId}`);
      }
      scheduleDbSave();
    }
  } catch (err) {
    console.warn('[Auto Deactivate Error]:', err);
  }
